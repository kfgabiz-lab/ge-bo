...
/* ── 스타일 상수 ── */
const INPUT_SM =
  "w-full border border-slate-200 rounded px-1.5 py-1 text-[11px] text-slate-800 focus:outline-none focus:ring-1 focus:ring-slate-400 bg-white font-mono";
const SELECT_SM =
  "w-full border border-slate-200 rounded px-1.5 py-1 text-[11px] text-slate-800 focus:outline-none focus:ring-1 focus:ring-slate-400 bg-white cursor-pointer";
...
export function EntityFieldEditor({ entity, onDeleted, onUpdated }: Props) {
  ...
  const validate = (): boolean => {
    for (let i = 0; i < fields.length; i++) {
      const f = fields[i];
      if (!f.label.trim()) {
        toast.warning(`${i + 1}번 행: label을 입력해주세요.`);
        return false;
      }
      if (!f.columnType) {
        toast.warning(`${i + 1}번 행: DB 타입을 선택해주세요.`);
        return false;
      }
    }
    return true;
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden h-full min-h-0 flex flex-col">
      ...
      <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold ${entity.active ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-400"}`}
          >
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-slate-400">table_name</span>
          <div className="w-36">
            <input
              type="text"
              value={tableName}
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-slate-400">마스터 Entity</span>
          <div className="w-40">
            <select
                  ...
                  <td className="px-1 py-1">
                    {f.columnType === "ENTITY_REF" ? (
                      <AutocompleteSelectField<{ id: number; slug: string; name: string }, number | null>
                        ...
                        filterOption={(o, q) => o.slug.toLowerCase().includes(q) || o.name.toLowerCase().includes(q)}
      ...
      {saveResult && <CodeSaveResultModal result={saveResult} onClose={() => setSaveResult(null)} />}
    </div>
  );
}
