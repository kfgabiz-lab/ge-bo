...
/* ── 스타일 상수 ── */
const ITEM_SELECTED =
  "w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-left transition-all bg-slate-900 text-white";
const ITEM_DEFAULT =
  "w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-left transition-all hover:bg-slate-50 text-slate-700";
const INPUT_CLS =
...
export function EntityList({ selectedId, onSelect, onCreated }: Props) {
  ...
  useEffect(() => {
    api
      .get<{ id: number; slug: string; name: string }[]>("/slug-entity/active")
  ...
  const filtered = entities.filter(
    (e) => e.slug.toLowerCase().includes(search.toLowerCase()) || e.name.toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="flex-1 overflow-y-auto p-1.5">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-8 text-slate-400">
            <span className="text-[11px]">{search ? "검색 결과가 없습니다." : "등록된 entity가 없습니다."}</span>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-start flex-1 min-w-0">
                    <span className="text-[11px] font-mono font-semibold truncate w-full">{entity.slug}</span>
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">
                  ...
                  <span className="ml-2 text-sm font-normal text-slate-400">(등록 후 변경 불가)</span>
                </label>
                <input
                  type="text"
                  value={form.slug}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">
                  DB 테이블명
                  <span className="ml-2 text-sm font-normal text-slate-400">(선택, entity 생성 기능에 사용)</span>
                </label>
                <input
                  type="text"
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="entity-active" className="text-sm text-slate-700 cursor-pointer">
                  사용
                </label>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-100">
              <button
                ...
                className="px-4 py-2 text-sm font-semibold text-slate-600 border border-slate-200 rounded-md hover:bg-slate-50"
              >
                취소
              </button>
              <button
