  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        <div>
          ...
          <input
            ...
            placeholder={t("code.placeholder.groupCode")}
            maxLength={30}
          />
        </div>
        <div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            placeholder={t("code.placeholder.description")}
            maxLength={200}
          />
        </div>
      </div>
      <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-slate-100 bg-slate-50/50">
        <button
          onClick={onCancel}
          className="px-4 py-2 text-sm font-medium text-slate-600 border border-slate-200 rounded-md hover:bg-white transition-all"
        >
          {t("common.btn.cancel")}
        </button>
        <button
...
export function CodeDetail() {
  ...
  const handleDelete = async () => {
    ...
    if (
      !confirm(
        `'${selectedGroup.groupName}' 그룹을 삭제하시겠습니까?\n하위 코드 ${selectedGroup.details?.length || 0}개도 함께 삭제됩니다.`
      )
    )
      return;
    setIsSubmitting(true);
      ...
      <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isSubmitting ? t("common.loading") : t("common.btn.save")}
          </button>
        </div>
      </div>
