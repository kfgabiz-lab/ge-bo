      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex items-center gap-3 bg-white border border-slate-200 rounded-lg px-4 py-1.5">
          <div className="flex-1 flex items-center gap-2">
            <label className="text-sm font-medium text-slate-600 shrink-0">{t("code.title.detail")}</label>
            <input
              type="text"
              value={search}
        ...
        <table className="w-full min-w-[900px]">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="px-3 py-2 text-[11px] font-semibold text-slate-600 text-left w-[120px]">
                {t("common.label.code")}
              </th>
              <th className="px-3 py-2 text-[11px] font-semibold text-slate-600 text-left w-[220px]">
                {t("common.label.codeName")}
              </th>
              <th className="px-3 py-2 text-[11px] font-semibold text-slate-600 text-center w-[50px]">
                {t("common.label.sort")}
              </th>
              {EXTRAS.map((_, i) => (
                <th
                  key={i}
                  className="px-2 py-2 text-[11px] font-semibold text-slate-600 text-left w-[90px]"
                >{`${t("common.label.extra")}${i + 1}`}</th>
              ))}
              <th className="px-3 py-2 text-[11px] font-semibold text-slate-600 text-center w-[50px]">
                {t("common.label.isActive")}
              </th>
              <th className="px-3 py-2 text-[11px] font-semibold text-slate-600 text-center w-[60px]">
                {t("common.label.manage")}
              </th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr className="bg-blue-50/50 border-b border-slate-100">
                ...
                <td className="px-2 py-1.5 text-center">
                  <div className="flex items-center justify-center gap-1">
                    <button onClick={handleAdd} className="p-1 rounded text-emerald-500 hover:bg-emerald-50">
            ...
            {filtered.map((d) => {
              ...
              return (
                <tr
                  key={d.id}
                  className={`border-b border-slate-100 ${isEditing ? "bg-amber-50/50" : "hover:bg-slate-50/50"}`}
                >
                  {isEditing ? (
                    /* 편집 모드 */
                    <>
                    ...
                    <>
                      ...
                      <td className="px-3 py-2 text-xs text-slate-700">{d.nameMsgKey ? t(d.nameMsgKey) : d.name}</td>
                      <td className="px-3 py-2 text-xs text-slate-500 text-center">{d.sortOrder}</td>
                      {EXTRAS.map((key) => (
                        <td key={key} className="px-2 py-2 text-xs text-slate-500 truncate max-w-[90px]">
                          {d[key] || "-"}
                        </td>
                      ))}
