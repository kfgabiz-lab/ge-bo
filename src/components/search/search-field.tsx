...
export function SearchField({ label, required, description, colSpan = 1, children }: SearchFieldProps) {
  return (
    <div className={COL_SPAN_MAP[colSpan]}>
      {label && (
        <label className="block text-sm font-semibold text-slate-700 flex-shrink-0">
          {label}
          {required && <span className="text-red-500 ml-0.5">*</span>}
        </label>
      )}
      {/* 설명 — description 유무와 무관하게 항상 동일 높이 예약 → input 위치 정렬 */}
      <p className="text-sm text-slate-400 mb-1 leading-tight truncate min-h-[18px]">{description}</p>
      {children}
    </div>
  );
}
