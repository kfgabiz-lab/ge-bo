...
import { isEnterSearchTrigger } from "./enter-search";
...
export function SearchForm({
  ...
  const { t } = useI18n();
  const resolvedSearchLabel = searchLabel ?? t("common.btn.search");
  ...
  return (
    <div className="bg-white rounded-md border border-slate-200">
      ...
      {collapsible && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-slate-700">{t("common.search.detail")}</span>
          </div>
          {isExpanded ? (
      ...
      {(!collapsible || isExpanded) && (
        <>
          {/* Enter 검색 — 필드 영역에서만 발화. 루트에 달면 검색/초기화/접기 버튼과 이중 실행됨 */}
          <div
            className={`px-4 space-y-3 ${collapsible ? "pt-3 pb-4 border-t border-slate-100" : "py-3"}`}
            onKeyDown={(e) => {
          ...
          {!hideActions && (
            <div className="flex items-center justify-end gap-2 px-4 py-2 bg-white border-t border-slate-100 rounded-b-xl">
              ...
              >
                <Search className="w-3 h-3" /> {resolvedSearchLabel}
              </button>
            </div>
          )}
        </>
