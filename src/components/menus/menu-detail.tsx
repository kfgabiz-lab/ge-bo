...
function TemplateUrlPicker({ onSelect }: { onSelect: (url: string, name: string) => void }) {
  ...
  const [list, setList] = useState<
    { id: number; name: string; slug: string; pageUrl: string; templateType?: string; configJson?: string }[]
  >([]);
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (ref.current && !ref.current.contains(e.target as Node) && drop && !drop.contains(e.target as Node)) {
        setOpen(false);
        setQuery("");
      }
    };
  ...
  const filtered = list.filter(
    (tpl) =>
      tpl.name.toLowerCase().includes(query.toLowerCase()) || tpl.slug.toLowerCase().includes(query.toLowerCase())
  );

  /* URL 생성 헬퍼 */
  const resolveUrl = (tpl: (typeof list)[0]) => {
    const isWidgetType =
      tpl.templateType === "PAGE" || tpl.templateType === "QUICK_LIST" || tpl.templateType === "QUICK_DETAIL";
    const isSinglePage =
      isWidgetType &&
      (() => {
        try {
          return !!JSON.parse(tpl.configJson || "{}").singlePage;
        } catch {
          return false;
        }
      })();
    return isWidgetType ? (isSinglePage ? `/admin/widgetSub/${tpl.slug}` : `/admin/widget/${tpl.slug}`) : tpl.pageUrl;
  };
                  ...
                  <li key={tpl.id}>
                    ...
                    >
                      <div className="flex items-center gap-1.5">
                        ...
                        {badge && (
                          <span
                            className={`flex-shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded ${badge.cls}`}
                          >
                            {badge.label}
                          </span>
                        )}
...
const validateUrl = (value: string, t: (key: string) => string): string => {
  ...
  /* 외부 링크(http/https)는 내부 상대경로 규칙(연속 슬래시 금지·제한 문자셋) 대신 EXTERNAL_URL_REGEX로 검증 */
  if (/^https?:\/\//.test(value)) {
    return EXTERNAL_URL_REGEX.test(value) ? "" : t("validation.url.pattern");
  }
      ...
      {showMeta && (
        <>
          <div>
            <label className="text-sm font-medium text-slate-600 mb-1.5 flex items-center gap-1">
              Meta Title
              <span className="ml-1.5 text-sm text-slate-400 font-normal">50~60자 사이로 입력해 주세요.</span>
            </label>
            <input
              type="text"
              value={metaTitle}
          ...
          <div>
            <label className="text-sm font-medium text-slate-600 mb-1.5 flex items-center gap-1">
              Meta Description
              <span className="ml-1.5 text-sm text-slate-400 font-normal">150~160자 사이로 입력해 주세요.</span>
            </label>
            <textarea
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden h-full flex flex-col">
      ...
      <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isSubmitting ? t("common.btn.saving") : t("menu.btn.add")}
          </button>
        </div>
      </div>
      ...
      <MenuForm
        ...
        onNameChange={(v) => {
          setName(v);
          if (nameMsgKeyError) setNameMsgKeyError("");
        }}
        onNameMsgKeyChange={(v) => {
          setNameMsgKey(v);
          if (nameMsgKeyError) setNameMsgKeyError("");
        }}
        ...
        onSortOrderChange={(v) => {
          ...
          if (!isNaN(n)) setSortOrder(n);
        }}
        onVisibleChange={setVisible}
        onTemplateSelect={(v, n) => {
          setUrl(v);
          setUrlError("");
          setLinkedTemplateName(n);
        }}
...
export function MenuDetail() {
          ...
          try {
            if (templatesCache.current.length === 0) {
              ...
              templatesCache.current = (
                ...
                .map((t) => {
                  const isSinglePage =
                    isWidget(t.templateType) &&
                    (() => {
                      try {
                        return !!JSON.parse(t.configJson || "{}").singlePage;
                      } catch {
                        return false;
                      }
                    })();
  ...
  const handleSortChange = (v: string) => {
    ...
    if (v === "") {
      setSortOrder("");
      setSortOrderError(t("validation.sort.required"));
      return;
    }
  ...
  const handleDelete = async () => {
    if (isSubmitting) return;
    const confirmMsg =
      t("menu.confirm.delete", { name: selectedMenu.name }) +
      ...
      <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isSubmitting ? t("common.btn.saving") : t("common.btn.save")}
          </button>
        </div>
      </div>
