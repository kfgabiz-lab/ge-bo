...
export function MenuTree() {
  const { menus, activeTab, setActiveTab, selectedMenu, startCreate, localUpdateMenuTree, __syncQueryMenus } =
    useMenuStore();
  ...
  useEffect(() => {
    if (serverMenus) {
      __syncQueryMenus(serverMenus, []);
    }
  }, [serverMenus, __syncQueryMenus]);
  ...
  const activeItem = useMemo(() => flattenedItems.find(({ id }) => id === activeId), [activeId, flattenedItems]);
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden h-full min-h-0 flex flex-col">
      <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 space-y-2">
        ...
        >
          ...
          {selectedMenu
            ? t("menu.btn.add_under", {
                name: selectedMenu.nameMsgKey ? t(selectedMenu.nameMsgKey) : selectedMenu.name,
              })
            : t("menu.btn.add")}
        </button>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {isMenusLoading ? (
          ...
          >
            <SortableContext items={itemsToRender.map((i) => i.id.toString())} strategy={verticalListSortingStrategy}>
              <div className="space-y-0.5">
                {itemsToRender.map((item) => (
                  <SortableTreeNode key={item.id} item={item} indentationWidth={INDENTATION_WIDTH} />
                ))}
              </div>
            </SortableContext>

            <DragOverlay
              dropAnimation={{ sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: "1" } } }) }}
            >
              {activeItem ? (
                <SortableTreeNode
                  item={{
