...
export function SlugSelector({ value, onChange, slugOptions, placeholder = "slug 선택", disabled }: Props) {
  ...
  const filtered = slugOptions.filter(
    (opt) =>
      opt.name.toLowerCase().includes(search.toLowerCase()) || opt.slug.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const triggerCls =
    "w-full border border-slate-200 rounded-md px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all flex items-center justify-between text-left disabled:opacity-50 disabled:cursor-not-allowed";

  return (
    <div className="relative">
      {/* 선택값 표시 / 드롭다운 토글 */}
      <button ref={triggerRef} type="button" disabled={disabled} onClick={handleToggle} className={triggerCls}>
      ...
      {/* 선택된 slug 힌트 */}
      {value && <p className="mt-1 text-sm text-slate-400 font-mono leading-tight">{value}</p>}
    </div>
  );
}
