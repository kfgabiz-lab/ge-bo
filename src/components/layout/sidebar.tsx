...
const MenuItemComponent = ({
  ...
  const inner = (
    <>
      ...
      <div className={`flex items-center gap-2.5 flex-1 min-w-0 ${isCollapsed ? "justify-center" : ""}`}>
        <span
          className={`flex-shrink-0 ${isActive ? "text-emerald-400" : "text-slate-400 group-hover:text-slate-200"} transition-colors`}
        >
  ...
  return (
    <div>
      {hasChildren || !hasUrl ? (
        <button onClick={handleToggle} className={`${base} ${style} w-full`}>
          {inner}
        </button>
      ) : (
        <Link
            ...
            <>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-slate-500 truncate">
                  {adminInfo?.roleDisplayNameMsgKey
                    ? t(adminInfo.roleDisplayNameMsgKey)
