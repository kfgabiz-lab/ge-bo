...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isLoginPage) {
    return <main className="min-h-screen bg-[#0A1F4E]">{children}</main>;
  }
  ...
  return (
    <AuthProvider>
      ...
      <div className="flex min-h-screen">
        <Sidebar />
        <div
          className={`flex-1 flex flex-col bg-gray-50 min-w-0 overflow-hidden transition-all duration-300 ${isSidebarCollapsed ? "ml-[70px]" : "ml-[220px]"}`}
        >
          <Header />
