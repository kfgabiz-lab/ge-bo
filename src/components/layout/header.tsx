...
import { useSiteStore, Site } from "@/store/use-site-store";
...
function findMenuBreadcrumb(
  ...
  for (const item of menus) {
    ...
    if (item.children && item.children.length > 0) {
      const result = findMenuBreadcrumb(item.children, pathname, t, [...parents, { label, href: item.url || "#" }]);
...
function SiteSelector() {
  const { activeSiteId, setActiveSiteId, loadActiveSiteFromStorage, setSites } = useSiteStore();
  ...
  /* 내 계정에 연동된 사이트만 조회 + 사이트 없는 사용자 차단
       - useSiteStore.sites에 그대로 채워 넣어야, 사이트관리 화면에 안 들어가도
         getActiveSiteTimezone()(게시기간 상태 판정 등)이 로그인/사이트 전환 시점부터 바로 최신 timezone을 읽는다 */
  useEffect(() => {
...
export function Header() {
  ...
  const resolvedCrumbs = menuCrumbs?.map((c, i, arr) =>
    i === arr.length - 1 && !c.label.trim() && pageTitle ? { ...c, label: pageTitle } : c
  );
  /* 메뉴에 없는 페이지 — previousPath로 부모 메뉴 전체 크럼 탐색 */
  const parentCrumbs = !resolvedCrumbs && previousPath ? (findMenuBreadcrumb(navMenus, previousPath, t) ?? null) : null;
