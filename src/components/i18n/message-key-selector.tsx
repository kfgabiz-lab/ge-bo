...
export function MessageKeySelector({ value, onChange, disabled, resourceType, size = "md" }: Props) {
  ...
  useEffect(() => {
    ...
    api
      .get("/message-resources", { params })
      .then((res) => {
        setOptions(
          res.data.content.map((item: { key: string; ko: string; en: string | null }) => ({
            key: item.key,
            ko: item.ko,
            en: item.en ?? null,
          }))
  ...
  const displayText = selected ? (locale === "en" && selected.en ? selected.en : selected.ko) : "";
            ...
            <div className="max-h-52 overflow-y-auto">
              {filtered.length === 0 ? (
                ...
                filtered.map((opt) => (
                  ...
                  >
                    <span className="text-slate-900">{locale === "en" && opt.en ? opt.en : opt.ko}</span>
                    <span className="text-slate-400 font-mono text-[10px] truncate max-w-[200px]">{opt.key}</span>
                  </button>
                ))
              )}
            </div>
