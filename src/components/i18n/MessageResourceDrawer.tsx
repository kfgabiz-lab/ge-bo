...
import { useMessageResourceStore } from '@/store/useMessageResourceStore';
...
export const MessageResourceDrawer = () => {
    ...
    const createForm = useForm<CreateFormData>({
        ...
        defaultValues: { key: '', ko: '', en: '' },
    });

    /* 수정 폼 */
    const updateForm = useForm<UpdateFormData>({
        resolver: zodResolver(updateSchema),
        defaultValues: { ko: '', en: '', active: true },
    });
    ...
    useEffect(() => {
        if (isEdit && selectedItem) {
            ...
            createForm.reset({ key: '', ko: '', en: '' });
        }
    }, [selectedItem, isDrawerOpen]);

    /* 기본 검색 파라미터 (목록 새로고침용) */
    const defaultSearch = { key: '', ko: '', en: '', active: '전체', page: 0, size: 20 };

    /* 등록 제출 */
    const onCreateSubmit = async (data: CreateFormData) => {
        try {
            await createItem({ key: data.key, ko: data.ko, en: data.en });
    ...
    const onUpdateSubmit = async (data: UpdateFormData) => {
        ...
        try {
            await updateItem(selectedItem.id, { ko: data.ko, en: data.en, active: data.active });
            fetchItems(defaultSearch);
        } catch {
            /* 오류는 store에서 toast 처리 */
        }
