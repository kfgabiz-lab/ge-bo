...
interface MessageResourceState {
  ...
  lastParams: MessageResourceSearchParams | null;
...
export const useMessageResourceStore = create<MessageResourceState>((set, get) => ({
  /* 초기값 */
  items: [],
  totalElements: 0,
  totalPages: 0,
  ...
  fetchItems: async (params) => {
    set({ isLoading: true, lastParams: params });
    try {
  ...
  refetch: async () => {
    const { lastParams, fetchItems } = get();
    if (lastParams) await fetchItems(lastParams);
  },
