...
export interface MenuItem {
  id: number;
  name: string; // 한국어 메뉴명 (BE에서 message_resource ko 값으로 자동 설정)
  nameMsgKey?: string; // message_resource.key
  /** 페이지 상단에 표시되는 메뉴 설명 (BE에서 자동 설정) */
  description?: string;
  descriptionMsgKey?: string; // message_resource.key
  url: string;
  /** SEO Meta Title (FO 2Depth 이상 메뉴 전용) */
  metaTitle?: string;
  /** SEO Meta Description (FO 2Depth 이상 메뉴 전용) */
...
interface MenuStore {
  ...
  __syncQueryMenus: (
    ...
    queryRoles: { id: number; name: string; displayName: string }[],
    isNav?: boolean
  ) => void;
}
...
export const useMenuStore = create<MenuStore>((set, get) => ({
  ...
  startCreate: (parentId, parentDepth) =>
    set({ isCreating: true, createParentId: parentId, createMaxDepth: parentDepth, selectedMenu: null }),
  ...
  addMenu: async (menu) => {
    try {
      const res = await api.post(API_PATH, {
        ...
        url: menu.url || "",
        metaTitle: menu.metaTitle || undefined,
        metaDescription: menu.metaDescription || undefined,
        icon: menu.icon,
  ...
  updateMenu: async (id, updates) => {
    try {
      ...
      await api.put(`${API_PATH}/${id}`, {
        ...
        descriptionMsgKey: resolvedMsgKey
          ? "descriptionMsgKey" in updates
            ? updates.descriptionMsgKey
            : menu.descriptionMsgKey
          : undefined,
        url: updates.url ?? menu.url ?? "",
        metaTitle: ("metaTitle" in updates ? updates.metaTitle : menu.metaTitle) || undefined,
        metaDescription: ("metaDescription" in updates ? updates.metaDescription : menu.metaDescription) || undefined,
