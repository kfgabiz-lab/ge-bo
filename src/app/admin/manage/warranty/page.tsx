...
const DETAIL_PAGE_PATH = "/admin/manage/warranty/detail";
...
const EDIT_PAGE_RULES_Table1: { connType?: string; pageSlug?: string; passParam?: string; conditionParam?: string }[] =
  [
    {
      connType: "page",
      pageSlug: "warrantyPolicy-detail",
      passParam: "update=1",
      conditionParam: "",
    },
  ];
...
export default function GeneratedPage() {
  ...
  const { t } = useI18n();
  useEffect(() => {
    setPageTitle(t("common.label.warrantypolicy"));
  }, [setPageTitle, t]);
  ...
  const dataSlugTable1 = "warrantyPolicy-data";
                  ...
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10">
                      <tr className="border-b border-slate-200 bg-slate-50/80">
                        ...
                        >
                          <button
                            onClick={() => handleSortTable1("product_type")}
                            className="flex items-center justify-center gap-1 w-full transition-colors hover:text-slate-900"
                          >
                            {t("common.label.productType")}
                            {(sortKeyTable1 === "product_type" ? sortDirTable1 : false) === "asc" ? (
                              <ChevronUp className="w-3.5 h-3.5 text-blue-500" />
                            ) : (sortKeyTable1 === "product_type" ? sortDirTable1 : false) === "desc" ? (
                        ...
                        >
                          <button
                            onClick={() => handleSortTable1("product_name")}
                            className="flex items-center justify-center gap-1 w-full transition-colors hover:text-slate-900"
                          >
                            {t("common.label.productName")}
                            {(sortKeyTable1 === "product_name" ? sortDirTable1 : false) === "asc" ? (
                              <ChevronUp className="w-3.5 h-3.5 text-blue-500" />
                            ) : (sortKeyTable1 === "product_name" ? sortDirTable1 : false) === "desc" ? (
                              <ChevronDown className="w-3.5 h-3.5 text-blue-500" />
                            ) : (
                              <ChevronsUpDown className="w-3.5 h-3.5 text-gray-300" />
                            )}
                        ...
                        >
                          <button
                            onClick={() => handleSortTable1("warranty_period")}
                            className="flex items-center justify-center gap-1 w-full transition-colors hover:text-slate-900"
                          >
                            {t("warrantyPolicy.label.purchaseWarranty")}
                            {(sortKeyTable1 === "warranty_period" ? sortDirTable1 : false) === "asc" ? (
                              <ChevronUp className="w-3.5 h-3.5 text-blue-500" />
                            ) : (sortKeyTable1 === "warranty_period" ? sortDirTable1 : false) === "desc" ? (
                              <ChevronDown className="w-3.5 h-3.5 text-blue-500" />
                            ) : (
                              <ChevronsUpDown className="w-3.5 h-3.5 text-gray-300" />
                            )}
                      ...
                      {loadingTable1 ? (
                        ...
                        rowsTable1.map((row, idx) => (
                          ...
                          >
                            ...
                            >
                              {(() => {
                                const value = row["product_type"];
                                const strVal = value == null || typeof value === "object" ? "" : String(value);
                                const displayVal = resolveCodeLabel(strVal, "PRODUCTTYPE", "text", groups, t);
                                return (
                            ...
                            >
                              {(() => {
                                const value = row["warranty_period"];
                                const strVal = value == null || typeof value === "object" ? "" : String(value);
                                const displayVal = strVal;
                                return (
                                  <span className="text-sm text-slate-700 truncate block" title={displayVal}>
