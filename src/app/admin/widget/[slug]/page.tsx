...
interface WidgetConfig {
  ...
  /** entity 모드에서도 검색 파라미터를 서버로 전송할지 여부(opt-in) — useWidgetPageState의 entitySearchEnabled로 전달 */
...
export default function WidgetRendererPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { gridProps } = useWidgetPageState(widgetItems, slug, { connectedType, entitySearchEnabled: entitySearch });

  /* 템플릿 + 공통코드 로딩 */
  useEffect(() => {
    fetchGroups();
    api
      .get(`/page-templates/by-slug/${slug}`, { params: { type: "PAGE" } })
      .then((res) => {
