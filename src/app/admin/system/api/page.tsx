...
export default function ApiInfoPage() {
  ...
  const fetchList = useCallback(
    async (page = 0, cat = filterCategory, mth = filterMethod, act = filterActive, kw = filterKeyword) => {
      setLoading(true);
      try {
  ...
  const handleSave = async () => {
    if (!form.name.trim()) {
      toast.warning("API 명칭을 입력해주세요.");
      return;
    }
    if (!form.urlPattern.trim()) {
      toast.warning("URL 패턴을 입력해주세요.");
      return;
    }
  ...
  return (
    <div className="h-full flex flex-col">
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0">
              <tr className="bg-slate-50/90 border-b border-slate-200 backdrop-blur-sm">
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap w-[110px]">
                  카테고리
                </th>
                ...
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-center whitespace-nowrap w-[70px]">
                  사용
                </th>
                ...
                items.map((item) => (
                  <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${METHOD_CLS[item.method] ?? "bg-slate-100 text-slate-600"}`}
                      >
                        {item.method}
                      </span>
                    </td>
                    <td
                      className="px-4 py-3 text-xs font-mono text-slate-600 max-w-[280px] truncate"
                      title={item.urlPattern}
                    >
                      {item.urlPattern}
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center">
                        <ToggleSwitch checked={item.active} onChange={(next) => handleToggleActive(item, next)} />
                      </div>
                    </td>
                    <td className="px-4 py-3 text-center">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-900">{editTarget ? "API 정보 수정" : "API 정보 등록"}</h2>
              <button
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-600 mb-1.5">
                    메서드 <span className="text-red-400">*</span>
                  </label>
                  <div className="relative">
                    <select
                      value={form.method}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">
                  API 명칭 <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={form.name}
                  onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                  placeholder="예: 게시판 목록 조회"
                  className={inputCls}
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">
                  URL 패턴 <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={form.urlPattern}
                  onChange={(e) => setForm((f) => ({ ...f, urlPattern: e.target.value }))}
                  placeholder="예: /api/v1/page-data/{slug}"
              ...
              {/* 설명 */}
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">설명</label>
                <textarea
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="active-check" className="text-sm text-slate-700 cursor-pointer">
                  사용
                </label>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-100">
              <button
                onClick={() => setModalOpen(false)}
                className="px-4 py-2 text-sm font-semibold text-slate-600 border border-slate-200 rounded-md hover:bg-slate-50 transition-all"
              >
                취소
              </button>
              <button
      ...
      {/* Entity 정보 팝업 */}
      {popupEntity && <EntityInfoPopup entityName={popupEntity} onClose={() => setPopupEntity(null)} />}
    </div>
  );
}
