...
/* 공통 input/select 스타일 */
const inputCls =
  "w-full border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white";
const selectCls =
  "w-full appearance-none border border-slate-200 rounded-md px-3 py-2 pr-8 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white cursor-pointer";
...
export default function SlugRelationPage() {
  ...
  const fetchSlugs = useCallback(async () => {
    try {
      ...
      setSlugOptions(
        res.data.filter((d) => d.type === "PAGE_DATA").map((d) => ({ id: d.id, slug: d.slug, name: d.name }))
      );
    } catch {
      /* 목록 로드 실패해도 무시 */
    }
  ...
  const fetchList = useCallback(
    async (page = 0, master = filterMaster, slave = filterSlave, dir = filterDir) => {
      setLoading(true);
      try {
  ...
  const handleSave = async () => {
    if (!form.masterSlug.trim()) {
      toast.warning("Master Slug를 입력해주세요.");
      return;
    }
    if (!form.slaveSlug.trim()) {
      toast.warning("Slave Slug를 입력해주세요.");
      return;
    }
    if (!form.slaveKey.trim()) {
      toast.warning("Slave Key를 입력해주세요.");
      return;
    }
    ...
    try {
      const body = {
        ...
        categoryDepthFrom:
          form.slaveType === "CATEGORY" && form.categoryDepthFrom.trim()
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        <button
          ...
          className="px-4 py-2 bg-slate-900 text-white text-sm font-semibold rounded-md hover:bg-slate-800 transition-all"
        >
          검색
        </button>
        <button
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0">
              <tr className="bg-slate-50/90 border-b border-slate-200 backdrop-blur-sm">
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-center whitespace-nowrap w-[60px]">
                  ID
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap">
                  Master Slug
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap">
                  Slave Slug
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap w-[130px]">
                  Master Key
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap w-[130px]">
                  Slave Key
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-center whitespace-nowrap w-[120px]">
                  Join Type
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap w-[130px]">
                  Slave Filter
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-center whitespace-nowrap w-[90px]">
                  방향
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-center whitespace-nowrap w-[100px]">
                  Slave Type
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap">설명</th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-center whitespace-nowrap w-[80px]">
                  관리
                </th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-medium ${DIR_CLS[item.relationDir] ?? "bg-slate-100 text-slate-600"}`}
                      >
                        {item.relationDir}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      {item.relationDir === "FETCH" && (
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium ${item.slaveType === "CATEGORY" ? "bg-purple-100 text-purple-700" : "bg-slate-100 text-slate-600"}`}
                        >
                    ...
                    <td
                      className="px-4 py-3 text-xs text-slate-600 max-w-[240px] truncate"
            ...
            {Array.from({ length: totalPages }, (_, i) => i).map((p) => (
              <button
                ...
                className={`px-2.5 py-1 text-xs rounded transition-colors ${
                  p === currentPage ? "bg-slate-900 text-white font-semibold" : "text-slate-500 hover:bg-slate-100"
                }`}
              >
                {p + 1}
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-900">{editTarget ? "SLUG 연동 수정" : "SLUG 연동 등록"}</h2>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <SlugSelector
                    ...
                    placeholder="slug 선택"
                  />
                  <p className="mt-1 text-sm text-slate-400">기준 슬러그 (조회 대상)</p>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                  ...
                  <SlugSelector
                    ...
                    placeholder="slug 선택"
                  />
                  <p className="mt-1 text-sm text-slate-400">관계 슬러그 (필터링/fetch 기준)</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={inputCls}
                  />
                  <p className="mt-1 text-sm text-slate-400">master에서 비교할 필드 경로</p>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                    Slave Key <span className="text-red-500">*</span>
                  </label>
                  <input
                    value={form.slaveKey}
                    ...
                    className={inputCls}
                  />
                  <p className="mt-1 text-sm text-slate-400">slave에서 master를 참조하는 필드 경로</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">Join Type</label>
                  <div className="relative">
                    <select
                      value={form.joinType}
                      onChange={(e) => setField("joinType", e.target.value)}
                      className={selectCls}
                    >
                      {JOIN_TYPES.map((t) => (
                  ...
                  <p className="mt-1 text-sm text-slate-400">EQ: 단순 동등 / ARRAY_CONTAINS: 배열 포함</p>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">방향 (Relation Dir)</label>
                  <div className="relative">
                    <select
                      value={form.relationDir}
                      onChange={(e) => setField("relationDir", e.target.value)}
                      className={selectCls}
                    >
                      {RELATION_DIRS.map((d) => (
                        <option key={d} value={d}>
                  ...
                  <p className="mt-1 text-sm text-slate-400">FILTER: slave→master 필터링 / FETCH: slave 포함 반환</p>
                </div>
              </div>

              {/* Slave Filter */}
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">Slave Filter</label>
                <input
                  ...
                  className={inputCls}
                />
                <p className="mt-1 text-sm text-slate-400">slave 조회 시 고정 조건 (없으면 빈 값)</p>
              </div>

              {/* Slave Type — 항상 표시 */}
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">Slave Type</label>
                <div className="relative">
                  <select
                    value={form.slaveType}
                    onChange={(e) => setField("slaveType", e.target.value)}
                    className={selectCls}
                  >
                    {SLAVE_TYPES.map((t) => (
                      <option key={t} value={t}>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  TABLE: slave에서 직접 추출 / CATEGORY: 상위 계층 거슬러 추출
                </p>
              </div>
              ...
              {form.slaveType === "CATEGORY" && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">Category Depth (끝)</label>
                    <input
                      type="number"
                      min={1}
                      max={5}
                      ...
                      className={inputCls}
                    />
                    <p className="mt-1 text-sm text-slate-400">표시할 마지막 계층 (최대 5)</p>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                      시작 Depth <span className="text-slate-300 font-normal">(선택)</span>
                    </label>
                    <input
                      type="number"
                      min={1}
                    ...
                    />
                    <p className="mt-1 text-sm text-slate-400">
                      비우면 끝 Depth 하나만, 값 있으면 시작~끝 범위를 &quot; &gt; &quot;로 합쳐 표시 (예: 1~2 =
                      &quot;대분류 &gt; 중분류&quot;). 매칭 레코드가 여러 건이면 그 사이는 Fetch Separator로 구분
                    </p>
                  </div>
                </div>
              )}
              ...
              {form.relationDir === "FETCH" && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">Fetch Fields</label>
                    <input
                      ...
                      className={inputCls}
                    />
                    <p className="mt-1 text-sm text-slate-400">표시할 slave 필드 경로 (dot notation)</p>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">Fetch Separator</label>
                    <input
                      ...
                      className={inputCls}
                    />
                    <p className="mt-1 text-sm text-slate-400">여러 값 구분자 (기본: ,)</p>
                  </div>
                </div>
              )}
