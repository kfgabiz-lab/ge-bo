...
const readonlyCls =
  "w-full border border-slate-100 rounded-md px-3 py-2 text-sm text-slate-500 bg-slate-50 font-mono cursor-not-allowed";
...
export default function SlugRegistryPage() {
  ...
  useEffect(() => {
    api
      .get<SlugEntityOption[]>("/slug-entity/active")
      .then((res) => setEntityOptions(res.data))
      .catch(() => {
        /* entity 목록 실패 시 select box 비움 — 무시 */
      });
  }, []);

  /* ── 목록 조회 ── */
  const fetchList = useCallback(
    async (page = 0, typ = filterType, kw = filterKeyword) => {
      setLoading(true);
      try {
  ...
  const handleSave = async () => {
    if (!form.slug.trim()) {
      toast.warning("slug를 입력해주세요.");
      return;
    }
    if (!form.name.trim()) {
      toast.warning("slug 별칭을 입력해주세요.");
      return;
    }
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0">
              <tr className="bg-slate-50/90 border-b border-slate-200 backdrop-blur-sm">
                ...
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-center whitespace-nowrap w-[130px]">
                  타입
                </th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap w-[140px]">
                  Entity
                </th>
                ...
                <th className="px-4 py-3 text-xs font-semibold text-slate-600 text-left whitespace-nowrap w-[90px]">
                  생성자
                </th>
                ...
                items.map((item) => (
                  <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${TYPE_CLS[item.type] ?? "bg-slate-100 text-slate-600"}`}
                      >
                        {item.type}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-900">{editTarget ? "Slug 수정" : "Slug 등록"}</h2>
              <button
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-1.5">
                  ...
                  {editTarget && <span className="ml-2 text-sm font-normal text-slate-400">(등록 후 변경 불가)</span>}
                </label>
                {editTarget ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-semibold text-slate-600 mb-1.5">
                    별칭 <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.name}
              ...
              {form.type === "PAGE_DATA" && (
                <div>
                  <label className="block text-sm font-semibold text-slate-600 mb-1.5">
                    Entity 연결
                    <span className="ml-1.5 text-sm font-normal text-slate-400">(선택)</span>
                  </label>
                  <div className="relative">
                    <select
                      value={form.entityId ?? ""}
