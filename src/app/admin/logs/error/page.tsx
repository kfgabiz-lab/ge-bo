...
export default function ErrorLogPage() {
  ...
  const buildInitialSearch = (): Record<string, string> => ({
    ...
    f2: "",
    f3: "",
    f4_from: searchParams.get("dateFrom") ?? "",
    f4_to: searchParams.get("dateTo") ?? "",
  });
  ...
  const fetchList = useCallback(
    async (page = 0, search = appliedSearch, sk = sortKey, sd = sortDir) => {
      ...
      try {
        ...
        const res = await api.get<PageResponse>("/error-logs", { params });
  ...
  const SEARCH_WIDGET: SearchWidget = useMemo(
    () => ({
      type: "search",
      widgetId: "error-log-search",
      contentKey: "errorLogSearch",
  ...
  const TABLE_WIDGET: TableWidget = useMemo(
    () => ({
      type: "table",
      widgetId: "error-log-table",
      contentKey: "errorLogList",
      displayMode: "pagination",
      pageSize: PAGE_SIZE,
      connectedSearchIds: ["error-log-search"],
      connectedSlug: "",
      columns: [
        {
          id: "c1",
