 ...
 * - "지금"의 기준은 활성 사이트(useSiteStore)의 timezone — 사이트 timezone이 없으면 브라우저 로컬 시각으로 폴백(getNowParts 참고)
 *   ⚠️ new Date().toISOString()은 UTC 기준이라 사용 금지 — 시차만큼(KST는 9시간) 어긋난 값이 나와
 *   dateRangeStatus 등에서 실제 시각과 다른 판정 결과를 만든다.
 * @example formatNowBySubType('datetime') // "2026-07-03T16:41" (input[type=datetime-local] value와 동일 포맷)
...
export const formatNowBySubType = (subType: DateSubType): string => {
  ...
  if (subType === "datetime") return `${YYYY}-${MM}-${DD}T${hh}:${mm}`;
  if (subType === "time") return `${hh}:${mm}`;
  if (subType === "timeSec") return `${hh}:${mm}:${ss}`;
  return `${YYYY}-${MM}-${DD}`;
};
 ...
 */
export const evalConditionExpr = (condition: string, resolveField: (key: string) => string | undefined): boolean =>
 ...
 */
export const buildKeyToId = (fields: { id: string; fieldKey?: string }[]): Record<string, string> => {
...
/** 비교 연산자 심볼 → t() 미제공 시 사용할 한국어 고정 안내 메시지 */
const COMPARE_OP_FALLBACK: Record<string, (label: string, compareLabel: string) => string> = {
  "<": (label, compareLabel) => `'${label}' 값은 '${compareLabel}' 값보다 이전이어야 합니다.`,
  "<=": (label, compareLabel) => `'${label}' 값은 '${compareLabel}' 값보다 이전이거나 같아야 합니다.`,
  ">": (label, compareLabel) => `'${label}' 값은 '${compareLabel}' 값보다 이후여야 합니다.`,
  ">=": (label, compareLabel) => `'${label}' 값은 '${compareLabel}' 값보다 이후이거나 같아야 합니다.`,
};

/**
 * date/dateRange 필드간 대소비교 검증 — Form(validateFormFields)·SubList(validateSubListRows) 공용 로직
 * - 비교값 중 하나라도 비어있으면 비교를 생략한다 (필수 여부는 required 검사가 별도로 처리)
 * - 날짜 문자열은 evalConditionOp의 사전식 비교(§evalConditionOp)를 그대로 재사용 — 새 비교 로직을 만들지 않음
 * @param sourceLabel 현재 필드 라벨 (오류 메시지 표시용)
 * @param sourceVal   현재 필드의 비교값
 * @param opSymbol    비교 연산자 심볼 — < / <= / > / >= ("현재 값 OP 대상 값")
 * @param targetLabel 비교 대상 필드 라벨
 * @param targetVal   비교 대상 필드의 비교값
 * @returns 통과 시 true, 실패 시 toast.warning 후 false
 */
function checkDateCompare(
  ...
  if (evalConditionOp(sourceVal, opSymbol, targetVal)) return true;
  const vars = { label: sourceLabel, compareLabel: targetLabel };
  toast.warning(t ? t(COMPARE_OP_MSG_KEY[opSymbol], vars) : COMPARE_OP_FALLBACK[opSymbol](sourceLabel, targetLabel));
  return false;
}

/** compareExpr 단일 조건 파싱 결과 — "[selfPart]연산자$targetKey[_from|_to]" */
interface CompareToken {
  /** 자신이 dateRange일 때 비교에 쓸 값 — from/to. 생략 시 date는 무시, dateRange는 from 기본 */
  selfPart?: "from" | "to";
  /** 비교 연산자 — < <= > >= */
  op: string;
  /** 비교 대상 필드의 key */
  targetKey: string;
  /** 대상이 dateRange일 때 비교에 쓸 값 — from/to. 생략 시 from 기본 */
  targetPart?: "from" | "to";
}

/** compareExpr 연산자 파싱 순서 — 2글자 연산자(<=, >=)를 먼저 감지해야 1글자(<, >)와 충돌하지 않음 (evalConditionOp의 CONDITION_OPS와 동일 원칙) */
const COMPARE_EXPR_OPS = ["<=", ">=", "<", ">"] as const;

/**
 * compareExpr 단일 토큰 파싱 — "[part]연산자$fieldKey[_from|_to]" 문법
 * - target(대상 필드) 쪽은 이 프로젝트의 dateRange 저장키·검색 파라미터 관례(`fieldKey_from`/`fieldKey_to`)를 그대로 따른다
 *   (docs/ge_guide/builder/00-3.builder_field_reference.md §0-2/§0-3 참고)
 * - self(자신) 쪽 prefix(`from`/`to`)는 이 관례와 무관하게 compareExpr 전용으로 도입된 표기다
 * @example parseCompareToken("<$endDate")         // { op: '<', targetKey: 'endDate' }
 * @example parseCompareToken("to<$endDate_from")  // { selfPart: 'to', op: '<', targetKey: 'endDate', targetPart: 'from' }
 * @returns "연산자 뒤 $fieldKey" 형식이 아니면 null (해당 조건은 조용히 무시됨)
 */
function parseCompareToken(token: string): CompareToken | null {
  for (const op of COMPARE_EXPR_OPS) {
    const idx = token.indexOf(op);
    if (idx === -1) continue;
    const before = token.slice(0, idx).trim();
    const after = token.slice(idx + op.length).trim();
    if (!after.startsWith("$"))
      continue; /* 이 위치는 진짜 연산자가 아님(예: '<=' 탐색 중 '<' 오매칭) — 다음 후보 연산자로 */

    if (before !== "" && before !== "from" && before !== "to") return null; /* 알 수 없는 접두 — 잘못된 토큰 */
    const rest = after.slice(1); /* 선행 $ 제거 */

    /* 대상필드 참조는 이 프로젝트의 dateRange 관례인 언더스코어 접미사(_from/_to)를 사용 — dot 표기법은 쓰지 않음 */
    let targetKey: string;
    let targetPart: "from" | "to" | undefined;
    if (rest.endsWith("_from")) {
      targetKey = rest.slice(0, -"_from".length);
      targetPart = "from";
    } else if (rest.endsWith("_to")) {
      targetKey = rest.slice(0, -"_to".length);
      targetPart = "to";
    } else {
      targetKey = rest;
    ...
    return {
      selfPart: before === "from" || before === "to" ? before : undefined,
      op,
      targetKey,
      targetPart,
    };
...
/**
 * compareExpr(콤마 AND 다중조건) 검증 — Form(validateFormFields)·SubList(validateSubListRows) 공용 로직.
 * 콤마로 연결된 조건을 순서대로 검사하다가 첫 위반에서 즉시 false를 반환한다(AND, short-circuit).
 * 대상 필드를 찾지 못하는 조건은 조용히 건너뛴다.
 * @param compareExpr  콤마 구분 조건식 (예: "<$endDate,<$dueDate")
 * @param sourceLabel  현재 필드 라벨 (오류 메시지 표시용)
 * @param getSelfValue part('from'|'to'|undefined) → 현재 필드의 비교값 (date 타입이면 part를 무시하고 그대로 반환하면 됨)
 * @param findTarget   대상 fieldKey → { label, getValue } | undefined (getValue도 동일하게 part를 받아 비교값 반환)
 * @example checkCompareExpr("<$endDate,<$dueDate", "시작일", () => val, (key) => ({ label: '종료일', getValue: () => endVal }))
 */
function checkCompareExpr(
  compareExpr: string,
  sourceLabel: string,
  getSelfValue: (part?: "from" | "to") => string,
  findTarget: (fieldKey: string) => { label: string; getValue: (part?: "from" | "to") => string } | undefined,
  t?: (key: string, vars?: Record<string, string>) => string
): boolean {
  for (const rawToken of splitTopLevelComma(compareExpr)) {
    const parsed = parseCompareToken(rawToken);
    if (!parsed) continue;
    const target = findTarget(parsed.targetKey);
    if (!target) continue;
    const sourceVal = getSelfValue(parsed.selfPart);
    const targetVal = target.getValue(parsed.targetPart);
    if (!checkDateCompare(sourceLabel, sourceVal, parsed.op, target.label, targetVal, t)) return false;
  }
  return true;
}
...
export const validateFormFields = (
  ...
  for (const f of fields) {
    ...
    const val = isRangeType ? (values[f.id + "_from"] || "").trim() : (values[f.id] || "").trim();
    ...
    if (f.required) {
      ...
      if (empty) {
        toast.warning(
          t ? t("common.validation.field_required", { label: String(label) }) : `'${label}' 항목은 필수 입력입니다.`
        );
        return false;
      }
    }
    if (val && !FILE_FIELD_TYPES.includes(f.type as (typeof FILE_FIELD_TYPES)[number])) {
      if (f.minLength && val.length < f.minLength) {
        toast.warning(
          t
            ? t("common.validation.min_length", { label: String(label), min: String(f.minLength) })
            : `'${label}' 항목은 최소 ${f.minLength}자 이상 입력해야 합니다.`
        );
        return false;
      }
      if (f.maxLength && val.length > f.maxLength) {
        toast.warning(
          t
            ? t("common.validation.max_length", { label: String(label), max: String(f.maxLength) })
            : `'${label}' 항목은 최대 ${f.maxLength}자까지 입력 가능합니다.`
        );
        return false;
      }
    }
    ...
    if (
      FILE_FIELD_TYPES.includes(f.type as (typeof FILE_FIELD_TYPES)[number]) &&
      f.maxFileCount &&
      fileCount > f.maxFileCount
    ) {
      toast.warning(
        t
          ? t("common.validation.max_file_count", { label: String(label), count: String(f.maxFileCount) })
          : `'${label}' 항목은 최대 ${f.maxFileCount}개까지 첨부 가능합니다.`
      );
      return false;
    }
    ...
    if (FILE_FIELD_TYPES.includes(f.type as (typeof FILE_FIELD_TYPES)[number]) && f.maxTotalSizeMB) {
      ...
      if (total > f.maxTotalSizeMB * 1024 * 1024) {
        toast.warning(
          t
            ? t("common.validation.max_total_size", { label: String(label), size: String(f.maxTotalSizeMB) })
            : `'${label}' 전체 파일 용량이 ${f.maxTotalSizeMB}MB를 초과합니다.`
        );
        return false;
      }
    }
    ...
    /* 필드간 대소비교 검증 — date/dateRange(레거시 yearMonth/yearMonthRange 포함) 전용, compareExpr 설정 시에만 동작.
           비교 대상은 같은 Form 내 필드로 한정(fields에서 fieldKey로 직접 탐색)
           (validateSubListRows와 동일하게 date/dateRange 계열 타입가드를 둬 방어 패턴을 통일) */
    const isDateCompareType =
      f.type === "date" || f.type === "yearMonth" || f.type === "dateRange" || f.type === "yearMonthRange";
    if (isDateCompareType && f.compareExpr) {
      const getSelfValue = (part?: "from" | "to") => (isRangeType ? (part === "to" ? valTo : val) : val);
      const findTarget = (key: string) => {
        const targetField = fields.find((x) => x.fieldKey === key);
        if (!targetField) return undefined;
        const targetIsRange = targetField.type === "dateRange" || targetField.type === "yearMonthRange";
        return {
          label: String(
            (targetField.labelMsgKey && t ? t(targetField.labelMsgKey) : targetField.label) ||
              targetField.fieldKey ||
              targetField.id
          ),
          getValue: (part?: "from" | "to") =>
            targetIsRange
              ? (values[targetField.id + (part === "to" ? "_to" : "_from")] || "").trim()
              : (values[targetField.id] || "").trim(),
        };
      };
      if (!checkCompareExpr(f.compareExpr, String(label), getSelfValue, findTarget, t)) return false;
    }
  }
  return true;
};
...
/**
 * SubList date/dateRange 컬럼의 "비교값" 조회 — 컬럼이 dateRange면 part(from/to)에 해당하는
 * 분리 키(rangeStartKey/rangeEndKey와 동일 컨벤션)에서, date면 col.key에서 그대로 읽는다.
 * part 미지정 시 dateRange는 from 기본, date는 애초에 part를 사용하지 않으므로 무관하다.
 */
function resolveSubListCompareValue(
  col: import("./components/renderer/types").SubListColumn,
  row: Record<string, unknown>,
  part?: "from" | "to"
): string {
  if (col.type !== "dateRange") return String(row[col.key] ?? "").trim();
  const key = part === "to" ? (col.key2 ? col.key2 : `${col.key}_to`) : col.key2 ? col.key : `${col.key}_from`;
 ...
 * @param allFormValues  페이지 내 모든 Form 위젯 통합 값 — compareExpr이 SubList 밖 Form 필드를 참조할 때 폴백용 (선택)
 * @param allKeyToId     페이지 내 모든 fieldKey → fieldId 역매핑 — 위와 동일 목적 (선택)
 * @param allFieldLabels 페이지 내 모든 fieldKey → 라벨(t 적용된 최종 문자열) — 오류 메시지 표시용 (선택)
 * @example if (!validateSubListRows(subListWidgets, subListRowsMap, subListFileMap, allFormValues, allKeyToId, allFieldLabels)) return;
 */
export const validateSubListRows = (
  ...
  for (const w of widgets) {
    ...
    if (w.required && rows.length === 0) {
      ...
      toast.warning(
        t ? t("common.validation.sublist_min_rows", { title }) : `'${title}' 항목은 최소 1개 이상 입력해야 합니다.`
      );
      return false;
    }
      ...
      for (const col of cols) {
        ...
        if (col.type === "file" || col.type === "image") {
          ...
          if (col.required && existingCount + newCount === 0) {
            toast.warning(
              t
                ? t("common.validation.row_required", { label: String(label), row: String(i + 1) })
                : `'${label}' 항목은 ${i + 1}번째 행의 필수 입력입니다.`
            );
            return false;
          }
          ...
          if (col.maxFileCount && existingCount + newCount > col.maxFileCount) {
            toast.warning(
              t
                ? t("common.validation.row_max_file_count", {
                    label: String(label),
                    row: String(i + 1),
                    count: String(col.maxFileCount),
                  })
                : `'${label}' 항목은 ${i + 1}번째 행에서 최대 ${col.maxFileCount}개까지 첨부 가능합니다.`
            );
            return false;
          }
          ...
          if (col.maxTotalSizeMB) {
            ...
            if (total > col.maxTotalSizeMB * 1024 * 1024) {
              toast.warning(
                t
                  ? t("common.validation.row_max_total_size", {
                      label: String(label),
                      row: String(i + 1),
                      size: String(col.maxTotalSizeMB),
                    })
                  : `'${label}' 항목은 ${i + 1}번째 행의 전체 파일 용량이 ${col.maxTotalSizeMB}MB를 초과합니다.`
              );
              return false;
            }
          }
          ...
          if (col.compareExpr) {
            ...
            const findTarget = (key: string) => {
              ...
              if (targetCol)
                return {
                  label: String(
                    (targetCol.labelMsgKey && t ? t(targetCol.labelMsgKey) : targetCol.label) || targetCol.key
                  ),
                  getValue: (part?: "from" | "to") => resolveSubListCompareValue(targetCol, row, part),
                };
              /* ② 못 찾으면 같은 페이지의 다른 Form 위젯 필드로 폴백 */
              const fid = allKeyToId?.[key];
              if (!fid || !allFormValues) return undefined;
              return {
                label: allFieldLabels?.[key] ?? key,
                getValue: (part?: "from" | "to") =>
                  (allFormValues[fid + (part === "to" ? "_to" : "_from")] ?? allFormValues[fid] ?? "").trim(),
              };
            };
            if (!checkCompareExpr(col.compareExpr, String(label), getSelfValue, findTarget, t)) return false;
          }
        } else {
          ...
          if (val) {
            if (col.minLength && val.length < col.minLength) {
              toast.warning(
                t
                  ? t("common.validation.row_min_length", {
                      label: String(label),
                      row: String(i + 1),
                      min: String(col.minLength),
                    })
                  : `'${label}' 항목은 ${i + 1}번째 행에서 최소 ${col.minLength}자 이상 입력해야 합니다.`
              );
              return false;
            }
            if (col.maxLength && val.length > col.maxLength) {
              toast.warning(
                t
                  ? t("common.validation.row_max_length", {
                      label: String(label),
                      row: String(i + 1),
                      max: String(col.maxLength),
                    })
                  : `'${label}' 항목은 ${i + 1}번째 행에서 최대 ${col.maxLength}자까지 입력 가능합니다.`
              );
              return false;
            }
            if (col.pattern) {
          ...
          /* 필드간 대소비교 검증 — date 컬럼 전용, compareExpr 설정 시에만 동작 (date는 part 개념이 없으므로 self는 항상 val 그대로) */
          if (col.type === "date" && col.compareExpr) {
            const getSelfValue = () => val;
            const findTarget = (key: string) => {
              /* ① 같은 SubList 컬럼 우선 탐색 */
              const targetCol = cols.find((c) => c.key === key);
              if (targetCol)
 ...
 */
export async function uploadFiles(files: File[], templateSlug: string, fieldKey: string): Promise<number[]> {
  const ids: number[] = [];
...
function addDotNotationKeys(target: Record<string, unknown>, obj: Record<string, unknown>, prefix: string): void {
...
export function evalColumnDataExpr(expr: string, row: Record<string, unknown>): string {
  ...
  if (qIdx !== -1) {
    ...
    if (pipeIdx !== -1) {
      ...
      return evalColumnDataExpr(evalRowCondition(condition, row) ? trueExpr : falseExpr, row);
    }
  }
 ...
 */
export function resolveFetchSeparator(rowData: Record<string, unknown>, relationSlugId: number | undefined): string {
  const DEFAULT_SEP = ",";
 ...
 */
export function codeDetailToLabel(detail: { name: string; nameMsgKey?: string }, t: (key: string) => string): string {
...
function maskPhone(value: string, pattern: string | undefined): string {
  ...
  if (parts.length === 3) {
    ...
    switch (pattern) {
      ...
      case "suffix4":
        return `${p1}-${p2}-${"*".repeat(p3.length)}`;
      case "midSuffix":
        return `${p1}-${"*".repeat(p2.length)}-${"*".repeat(p3.length)}`;
      default:
        return value;
    }
  }
...
export function applyMask(
  ...
  switch (maskType) {
    case "email":
      return maskEmail(value, maskPattern);
    case "phone":
      return maskPhone(value, maskPattern);
    case "name":
      return maskName(value, maskPattern);
    case "custom":
      return maskCustom(value, maskCustomRegex, maskCustomReplacement);
    default:
      return value;
  }
}
 ...
 */
export const getTemplateLabel = (t: { templateType?: string; configJson?: string; name: string }): string => {
...
export function resolveAccessor(obj: unknown, accessor: string): unknown {
  return accessor.split(".").reduce((acc: unknown, key) => {
    if (acc != null && typeof acc === "object" && !Array.isArray(acc)) return (acc as Record<string, unknown>)[key];
    return undefined;
  }, obj);
}
...
export function parseActionParams(
  ...
  const parts = quoteCount % 2 === 0 ? splitTopLevelComma(paramStr) : paramStr.split(",").map((p) => p.trim());
  parts.filter(Boolean).forEach((part) => {
    if (part.includes("=")) {
      ...
      const resolvedVal = literal !== null ? literal : val in row ? String(row[val] ?? "") : val;
...
export function validateDataSaveWidgets(opts: {
  ...
  if (
    !validateSubListRows(
      targetWidgets.filter((w) => w.type === "sublist") as Array<{
        type: string;
        widgetId?: string;
        required?: boolean;
        title?: string;
...
/**
 * Search 위젯 widgetId → 필드 목록 맵 — 페이지 레벨(useWidgetPageState)과
 * 팝업 레벨(WidgetRenderer) 양쪽에서 공유하기 위해 공용 함수로 추출.
 * (기존 useWidgetPageState.ts 내부 전용 함수와 동작 동일 — 코드만 이동)
 *
 * @example const fieldsMap = buildSearchFieldsMap(widgetItems);
 */
export function buildSearchFieldsMap(
  items: import("./components/renderer/PageGridRenderer").PageWidgetItem[]
): Record<string, import("./types").SearchFieldConfig[]> {
  const map: Record<string, import("./types").SearchFieldConfig[]> = {};
  items
    .flatMap((item) => item.contents.map((c) => c.widget))
    .forEach((w) => {
      if (w.type === "search") {
        map[w.widgetId] = (w.rows as { fields: import("./types").SearchFieldConfig[] }[]).flatMap((r) => r.fields);
      }
    });
  return map;
}
 ...
 * (기존 useWidgetPageState.ts fetchTableData 내부 로직을 그대로 이동 — 동작 동일)
 *
 * @param searchFields 검색 위젯(들)의 필드 정의 목록 (connectedSearchIds로 연결된 필드 전체)
 * @param sv           검색 값 (fieldId → 값, dateRange류는 `${fieldId}_from` / `${fieldId}_to`)
 * @example const params = buildSearchQueryParams(searchFields, searchValues);
 */
export function buildSearchQueryParams(
  searchFields: import("./types").SearchFieldConfig[],
