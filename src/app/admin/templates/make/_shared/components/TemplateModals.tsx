...
export const SaveModal = ({
  ...
  useEffect(() => {
    ...
    api
      .get("/slug-registry", { params: { type: "PAGE_TEMPLATE", size: "200", sort: "slug,asc" } })
      .then((res) => setSlugOptions(res.data?.content || []))
      .catch(() => {});
  }, [show]);

  /* 검색어로 필터링 */
  const filteredSlugOptions = slugOptions.filter(
    (o) =>
      o.slug.toLowerCase().includes(slugSearch.toLowerCase()) || o.name.toLowerCase().includes(slugSearch.toLowerCase())
  );
              ...
              {templateList.map((tpl) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold text-slate-800 group-hover:text-slate-900">{tpl.name}</span>
          ...
          <div className="flex items-end gap-2">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center border border-slate-200 rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-slate-900/10 focus-within:border-slate-900 transition-all">
                ...
                />
                <span className="px-2 py-2 text-xs text-slate-400 bg-slate-50 border-l border-slate-200 select-none">
                  .tsx
                </span>
              </div>
            </div>
      ...
      <div className="relative w-full max-w-lg bg-white rounded-xl shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          ...
          <div className="space-y-0.5">
            <label className="text-sm font-medium text-slate-500 block">연결 Slug</label>
            <select
