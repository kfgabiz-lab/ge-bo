  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 px-2 py-1.5">
        ...
        {/* 타입 배지 */}
        <span className="text-[10px] px-1 py-0.5 bg-slate-100 text-slate-500 rounded font-mono flex-shrink-0">
          {field.type}
        </span>
      ...
      {isEditing && <div className="px-2 pb-2 pt-1 border-t border-slate-100 space-y-2">{children}</div>}
    </div>
  );
}
...
export function FormBuilder({
  ...
  /**
   * date/dateRange 필드간 검증(compareExpr) 대상 후보가 존재하는지 여부 — DateField/DateRangeField의
   * "필드간 검증" 입력란을 노출할지 여부(존재 여부)에만 사용된다 (compareExpr은 자유 텍스트 입력이라 select는 없음)
   * - 같은 Form 내 자기 자신을 제외한 date·dateRange 계열 필드가 하나라도 있는지 확인 (yearMonth/yearMonthRange는 DateField/DateRangeField를 그대로 사용하므로 date/dateRange와 동일 취급)
   * - fieldKey가 없는 필드는 비교 대상으로 삼을 수 없으므로 제외
   */
  const hasDateCompareCandidates = (selfId: string) =>
    widget.fields.some(
      (other) =>
        other.id !== selfId &&
        other.fieldKey &&
        (other.type === "date" ||
          other.type === "yearMonth" ||
          other.type === "dateRange" ||
          other.type === "yearMonthRange")
    );
  ...
  const renderFieldComponent = (f: FormFieldItem) => {
    const props = {
      values: {
        ...
        compareExpr: f.compareExpr,
      } satisfies FieldEditValues,
      onChange: (updates: Partial<FieldEditValues>) => updateField(f.id, updates as Partial<FormFieldItem>),
    ...
    switch (f.type) {
      ...
      case "input":
        return <InputField {...props} fetchRelations={slugRelations.filter((r) => r.relationDir === "FETCH")} />;
      case "text":
        return <TextField {...props} fetchRelations={slugRelations.filter((r) => r.relationDir === "FETCH")} />;
      /* slugOptions: FormBuilder props에서 전달받은 PAGE_DATA slug 목록 — SLUG 탭 옵션 소스 선택에 사용 */
      case "select":
        return <SelectField {...props} slugOptions={slugOptions} />;
      /* hasCompareCandidates: 같은 Form 내 다른 date/dateRange 필드 존재 여부 — "필드간 검증" 섹션 노출에 사용 */
      case "date":
        return <DateField {...props} hasCompareCandidates={hasDateCompareCandidates(f.id)} />;
      case "dateRange":
        return <DateRangeField {...props} hasCompareCandidates={hasDateCompareCandidates(f.id)} />;
      case "yearMonth":
        return <DateField {...props} hasCompareCandidates={hasDateCompareCandidates(f.id)} />;
      case "yearMonthRange":
        return <DateRangeField {...props} hasCompareCandidates={hasDateCompareCandidates(f.id)} />;
      ...
      {!editingId &&
        (showPicker ? (
          <div className="border border-slate-200 rounded-md p-2 bg-slate-50/50">
            <FieldPickerTypeList types={FORM_FIELD_TYPES} onSelect={selectType} onCancel={() => setShowPicker(false)} />
          </div>
        ) : (
          <button
