...
export function OutputModePanel({
  ...
  onOutputModeChange,
  onPageTitleChange,
  onPageTitleMsgKeyChange,
  onLayerTypeChange,
  onLayerTitleChange,
  onLayerTitleMsgKeyChange,
  onLayerWidthChange,
    ...
    <>
      ...
      {outputMode === "page" && (
        <div className="border-b border-slate-100 bg-slate-50/30 px-3 py-3">
          <div className="flex items-center justify-between mb-1">
            <label className="text-sm font-semibold text-slate-500">페이지 제목</label>
            <div className="flex items-center gap-2">
              ...
              {onSinglePageChange && (
                <label className="flex items-center gap-1 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-sm font-semibold text-slate-500">단일페이지</span>
                </label>
              )}
              {/* 이탈체크 체크박스 */}
              {onLeaveCheckChange && (
                <label className="flex items-center gap-1 cursor-pointer select-none">
                  <input
                    ...
                    className="w-3.5 h-3.5 rounded border-slate-300 accent-slate-900 cursor-pointer"
                  />
                  <span className="text-sm font-semibold text-slate-500">이탈체크</span>
                </label>
              )}
              <button
        ...
        <div className="border-b border-slate-100 bg-slate-50/30 px-3 py-3">
          <div className={`flex gap-2 ${connectedType === "none" ? "" : "items-end"}`}>
            ...
            <div className={connectedType === "none" ? "w-full" : "w-[45%] shrink-0"}>
              <label className="text-sm font-semibold text-slate-500 mb-1 block">타입</label>
      ...
      {outputMode === "layerpopup" && (
        <div className="border-b border-slate-100 bg-blue-50/30 px-3 py-3 space-y-3">
          {/* 팝업 유형 */}
          <div>
            <label className="text-sm font-semibold text-slate-500 mb-1.5 block">팝업 유형</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-sm font-semibold text-slate-500">팝업 제목</label>
              <button
                onClick={() => setI18nMode((v) => !v)}
                className={`p-1 rounded transition-all ${i18nMode ? "text-blue-500 bg-blue-50" : "text-slate-400 hover:text-slate-600 hover:bg-slate-100"}`}
                title="다국어 모드"
          ...
          {layerType === "center" && (
            <div>
              <label className="text-sm font-semibold text-slate-500 mb-1 block">팝업 너비</label>
              <div className="relative">
                <select
                  value={layerWidth}
