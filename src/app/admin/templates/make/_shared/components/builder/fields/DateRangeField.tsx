...
const BTN_CLS =
  "w-6 h-7 text-xs font-bold rounded border border-slate-200 bg-white text-slate-500 hover:bg-slate-100 transition-colors flex-shrink-0";
...
const INPUT_NUM_CLS =
  "w-10 border border-slate-200 rounded px-1 py-1.5 text-xs text-center bg-white focus:outline-none focus:border-slate-900";
...
export function DateRangeField({
  ...
  /* 오늘날짜 토글은 시작/종료 각각 독립 — 켜진 쪽의 offset ±버튼·입력만 잠금 */
  const isStartTodayLocked = values.defaultStartToday === true;
  const isEndTodayLocked = values.defaultEndToday === true;
  ...
  const disablePastLabel = isTime
    ? "이전 시간 비활성화"
    : subType === "yearMonth"
      ? "이전 년월 비활성화"
      : "이전 날짜 비활성화";

  /* 서브타입 변경 시 날짜 기본값·오늘날짜(시작·종료 각각) 상태 초기화 */
  const handleSubTypeChange = (val: RangeSubType) => {
    onChange({
      ...
      defaultStartToday: undefined,
      defaultEndToday: undefined,
    });
  };
  ...
  return (
    <div className="space-y-1.5">
      ...
      <FieldBase
        ...
        label2MsgKey={values.label2MsgKey}
        showLabel2
      ...
      {/* 시작 기본값 */}
      <div>
        <label className={LABEL_CLS}>{startLabel}</label>
        {isTime ? (
          ...
          /* 나머지 타입: offset ±버튼 + 날짜 입력 — 시작 오늘날짜 ON 시 이 구간만 비활성화 */
          <div className="flex items-center gap-1">
            <button
              type="button"
              onClick={handleStartMinus}
              disabled={isStartTodayLocked}
            ...
            <input
              type="number"
              value={startOffset}
              onChange={handleStartOffsetInput}
              disabled={isStartTodayLocked}
            ...
            <div className={DATE_WRAP_CLS}>
              {isStartTodayLocked ? (
                ...
                <input
                  type={inputType}
                  value={calcOffsetValue(startOffset, subType)}
                  readOnly
                  className={INPUT_DATE_READONLY_CLS}
                />
              ) : (
        ...
        {/* 시작일 기본값 아래 줄 — 오늘날짜(시작 전용) · 이전날짜 비활성화(시작 전용) 나란히 배치 */}
        {!isTime && (
          <div className="flex items-center gap-3 mt-1.5">
            <ToggleRow
              label="오늘날짜"
              value={values.defaultStartToday ?? false}
              onChange={(v) => {
                /* 오늘날짜 ON 시 시작 offset·수동날짜만 초기화 — 종료 쪽은 영향 없음 */
                onChange({
                  defaultStartToday: v || undefined,
                  defaultStartDateOffset: v ? undefined : values.defaultStartDateOffset,
                  defaultStartDate: v ? undefined : values.defaultStartDate,
                });
              }}
            />
            <ToggleRow
              label={disablePastLabel}
              value={values.disableStartPast ?? false}
              onChange={(v) => onChange({ disableStartPast: v || undefined })}
            />
          </div>
        )}
      </div>
      ...
      {/* 종료 기본값 */}
      <div>
        <label className={LABEL_CLS}>{endLabel}</label>
        {isTime ? (
          /* 시간 계열: 직접 시간 입력 (offset 없음), timeSec는 초 단위 step=1 */
          <input
            type="time"
          ...
          /* 종료 오늘날짜 ON 시 이 구간만 비활성화 — 시작 구간과 독립 */
          <div className="flex items-center gap-1">
            <button
              type="button"
              onClick={handleEndMinus}
              disabled={isEndTodayLocked}
              className={`${BTN_CLS} disabled:opacity-40 disabled:cursor-not-allowed`}
            >
              -
            </button>
            ...
            <div className={DATE_WRAP_CLS}>
              {isEndTodayLocked ? (
                ...
                <input
                  type={inputType}
                  value={calcOffsetValue(endOffset, subType)}
                  readOnly
                  className={INPUT_DATE_READONLY_CLS}
                />
              ) : (
        ...
        {/* 종료일 기본값 아래 줄 — 오늘날짜(종료 전용) · 이전날짜 비활성화(종료 전용) 나란히 배치 */}
        {!isTime && (
          <div className="flex items-center gap-3 mt-1.5">
            <ToggleRow
              label="오늘날짜"
              value={values.defaultEndToday ?? false}
              onChange={(v) => {
                /* 오늘날짜 ON 시 종료 offset·수동날짜만 초기화 — 시작 쪽은 영향 없음 */
                onChange({
                  defaultEndToday: v || undefined,
                  defaultEndDateOffset: v ? undefined : values.defaultEndDateOffset,
                  defaultEndDate: v ? undefined : values.defaultEndDate,
                });
              }}
            />
            <ToggleRow
              label={disablePastLabel}
              value={values.disableEndPast ?? false}
              onChange={(v) => onChange({ disableEndPast: v || undefined })}
            />
          </div>
        )}
      </div>
      ...
      {/* 필드간 검증 — 다른 date/dateRange 필드와 대소비교(콤마구분 단일 표현식). hasCompareCandidates가 true일 때만(Form/SubList) 노출
                자신이 dateRange이므로 앞에 from</to< 접두를 붙여 시작일/종료일 중 비교에 쓸 값을 지정할 수 있다 */}
      {hasCompareCandidates && (
        <div className="space-y-1.5 pt-1.5 border-t border-slate-100">
          <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">필드간 검증</p>
          <div>
            <label className={LABEL_CLS}>비교식</label>
            <input
              type="text"
              value={values.compareExpr ?? ""}
              onChange={(e) => onChange({ compareExpr: e.target.value || undefined })}
              placeholder="to<$endDate_from"
              className={INPUT_CLS}
            />
          </div>
        </div>
