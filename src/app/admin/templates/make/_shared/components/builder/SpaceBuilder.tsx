...
function SortableSpaceItem({
  ...
  return (
    ...
    >
      {/* 아이템 헤더 */}
      <div
        className={`flex items-center gap-2 px-2.5 py-1.5 cursor-pointer select-none ${item.type === "textarea" ? "bg-amber-50" : "bg-orange-50"}`}
      ...
      >
        ...
        <span
          className={`text-[10px] font-semibold flex-1 ${item.type === "textarea" ? "text-amber-700" : "text-orange-700"}`}
        >
          {item.type === "textarea" ? (
      ...
      {/* 아이템 설정 (Accordion) */}
      {isEditing && <div className="p-2 border-t border-slate-100">{children}</div>}
    </div>
  );
}
...
export function SpaceBuilder({
  ...
  const resolvedContentWidgets: ContentWidgetOption[] =
    contentWidgets.length > 0
      ...
      : formWidgets.map((f) => ({
          ...
          connectedSlug: f.connectedSlug,
        }));
  const [editingId, setEditingId] = useState<string | null>(null);

  /* slug-relation 목록 — 엑셀 다운로드 ActionButton의 "JOIN 연결 Slug" 체크박스 선택에 사용 (FETCH 방향만) */
  ...
  const addItem = (type: "textarea" | "action-button") => {
    const newItem: SearchFieldConfig =
      type === "textarea"
        ? {
            id: uid(),
            type: "textarea",
            label: "",
            fieldKey: "",
            colSpan: maxColSpan as 1 | 2 | 3 | 4 | 5,
            rowSpan: 1,
            content: "",
            fontSize: 13,
            bold: false,
  ...
  /* 아이템 삭제 */
  const removeItem = (id: string) => onChange({ ...widget, items: widget.items.filter((i) => i.id !== id) });
              ...
              >
                ...
                {item.type === "action-button" && (
                  <ActionButtonField
                    ...
                    fetchRelations={slugRelations.filter((r) => r.relationDir === "FETCH")}
                  />
                )}
              </SortableSpaceItem>
            ))}
