...
export function CategoryField({
  ...
  const activeDepths: number[] = values.activeDepths ?? Array.from({ length: values.maxDepth ?? 1 }, (_, i) => i + 1);
  ...
  const handleToggleDepth = (d: number) => {
    const nextRaw = activeDepths.includes(d) ? activeDepths.filter((x) => x !== d) : [...activeDepths, d];
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="space-y-1">
        ...
        {activeDepths.map((depthNum, i) => (
          <div key={depthNum} className="space-y-0.5">
            ...
            <div className="grid grid-cols-[24px_1fr] gap-1 items-center">
              ...
              <span className="text-[10px] text-slate-400 text-right pr-0.5 shrink-0">{depthNum}</span>
                    ...
                    {parentFieldRequired && (
                      <label className={LABEL_CLS + " !mb-0.5"}>
                        <span className="text-amber-500 font-normal">
                          상위 부모 ID 경로 (옵션 사전필터 사용 시 필수 — 비우면 상위 옵션 0건)
                        </span>
                      </label>
                    )}
                    <input
                      ...
                      onChange={(e) =>
                        onChange({ depthParentFields: updateDepthArray(depthParentFields, i, e.target.value) })
                      }
                      placeholder={
                        parentFieldRequired
        ...
        <div className="text-[9px] text-slate-300 pt-0.5">경로 예시: id / dataJson.name / tab1.form1.title</div>
      </div>
    </div>
  );
}
