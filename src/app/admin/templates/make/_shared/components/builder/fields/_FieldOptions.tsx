        ...
        <div className="space-y-1.5">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[10px] text-slate-400 leading-tight">
              $fieldKey로 같은 Form/Search 위젯 내 다른 필드의 현재 선택값을 참조할 수 있습니다. (Form·Search 위젯에서만
              동작 — SubList 등 다른 곳에서는 무시됩니다)
            </p>
          </div>

          <div>
            <label className={LABEL_CLS}>
              옵션 파생 키 <span className="text-slate-300 font-normal">(선택)</span>
            </label>
            <input
              type="text"
              value={optionDerivedKeys ?? ""}
              onChange={(e) => onChange({ optionDerivedKeys: e.target.value || undefined })}
              placeholder="예: productCategory,productType"
              className={INPUT_CLS}
            />
          </div>
