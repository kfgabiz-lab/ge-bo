  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${TYPE_BADGE_CLS[field.type]}`}>{field.type}</span>
...
export function MultiSelectBuilder({
  ...
  /* "연동" 모드 후보 — FETCH 방향 relation만 (CategoryBuilder.tsx의 "연결 Slug" 후보 필터링과 동일한 패턴).
       slaveType은 레거시 relation에서 비어있을 수 있어(optional) 추가로 걸러내지 않는다 — CategoryBuilder도 동일. */
  ...
  /* ── 추가 필드 삭제 ── */
  const removeExtraField = (id: string) => onChange({ ...widget, extraFields: extraFields.filter((f) => f.id !== id) });
  ...
  return (
    <div className="space-y-5 pt-1">
      ...
      <section className="space-y-3">
          ...
          <div className="col-span-8">
            {(widget.sourceMode ?? "call") === "call" ? (
              ...
              <SlugSelectField
                ...
                slugOptions={sourceRelations.map((r) => ({
                  id: r.id,
                  slug: String(r.id),
                  name: r.description
        ...
        <div>
          <label className={LABEL_CLS}>
            옵션 필터 <span className="text-slate-300 font-normal">(선택)</span>
          </label>
          <input
            type="text"
        ...
        {(widget.sourceMode ?? "call") === "call" && (
          <div>
            <label className={LABEL_CLS}>
              표시 필드 <span className="text-red-400">*</span>
            </label>
            <input
              type="text"
