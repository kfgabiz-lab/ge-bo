 ...
 *
 * Input / FormTextarea / Editor 빌더 필드 하단에 공통으로 사용.
 * 추가 버튼으로 세트를 늘릴 수 있으며, 세트별로 독립된 생성KEY + 변환옵션을 제공한다.
 *
 * 사용법:
...
interface DataGenerationSectionProps {
  ...
  values: Pick<
    FieldEditValues,
    "generationKey" | "dataReplacement" | "caseChange" | "appendText" | "truncateLength" | "dataGenerations"
  >;
  /** 변경 핸들러 */
  onChange: (updates: Partial<FieldEditValues>) => void;
  /** 필드 타입 — 'editor'일 때 HTML제거|글자자르기 UI 표시 (기본: 'input') */
  fieldType?: "input" | "editor" | "date" | "dateRange";
}

/** select 공통 스타일 */
const SELECT_CLS =
  "w-full border border-slate-200 rounded px-2 py-1.5 text-xs focus:outline-none appearance-none bg-white";
...
function GenerationEntryRow({
  ...
  const isDateType = fieldType === "date" || fieldType === "dateRange";
  const isDateRangeType = fieldType === "dateRange";
  return (
    <div className="pl-2 border-l-2 border-slate-100 space-y-1.5">
      ...
      {isDateRangeType && (
        <div>
          <label className={LABEL_CLS}>시작/종료</label>
          <select
            value={entry.datePart ?? ""}
            onChange={(e) =>
              onChange({ ...entry, datePart: (e.target.value || undefined) as "from" | "to" | undefined })
            }
            className={SELECT_CLS}
          >
            <option value="">미지정</option>
            <option value="from">시작</option>
            <option value="to">종료</option>
          </select>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 gap-1.5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="0"
              onChange={(e) =>
                onChange({ ...entry, truncateLength: e.target.value ? Number(e.target.value) : undefined })
              }
              className={INPUT_CLS}
            />
          </div>
...
export function DataGenerationSection({ values, onChange, fieldType = "input" }: DataGenerationSectionProps) {
  ...
  const entries: DataGenerationEntry[] = values.dataGenerations?.length
    ...
    : values.generationKey
      ? [
          {
            generationKey: values.generationKey,
            dataReplacement: values.dataReplacement,
            caseChange: values.caseChange,
            appendText: values.appendText,
            truncateLength: values.truncateLength,
          },
        ]
      : [];
  ...
  const handleChange = (index: number, updated: DataGenerationEntry) => {
    ...
    onChange({
      dataGenerations: next,
      generationKey: undefined,
      dataReplacement: undefined,
      caseChange: undefined,
  ...
  const handleRemove = (index: number) => {
    ...
    onChange({
      dataGenerations: next.length ? next : undefined,
      generationKey: undefined,
      dataReplacement: undefined,
      caseChange: undefined,
      appendText: undefined,
  ...
  const handleAdd = () => {
    onChange({
      dataGenerations: [...entries, { ...EMPTY_ENTRY }],
      generationKey: undefined,
      dataReplacement: undefined,
      caseChange: undefined,
      appendText: undefined,
