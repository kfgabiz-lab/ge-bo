            ...
            {targetType === "slug" ? (
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-1">
                  <input
                    type="checkbox"
                    id="use-preview-token"
                    checked={values.usePreviewToken ?? false}
                    onChange={(e) => onChange({ usePreviewToken: e.target.checked })}
                  />
                  <label htmlFor="use-preview-token" className="text-xs text-slate-600">
                    미리보기 토큰 사용 (비공개 컨텐츠를 담당자만 FO에서 미리보기)
                  </label>
                </div>
              </div>
            )}
