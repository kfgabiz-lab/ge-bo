...
const ExtensionTagInput = ({ extensions, onChange }: { extensions: string[]; onChange: (exts: string[]) => void }) => {
  ...
  const addExt = () => {
    ...
    if (!/^\.[a-zA-Z0-9]+$/.test(ext)) {
      setError("영문/숫자 확장자만 입력 가능합니다. (예: .jpg)");
      return;
    }
    ...
    <div className="space-y-1">
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          onClick={addExt}
          disabled={!input.trim()}
          className="px-2 py-1 text-[10px] font-semibold bg-slate-900 text-white rounded hover:bg-slate-800 disabled:opacity-40 transition-all"
        >
          추가
        </button>
      </div>
      ...
      {extensions.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {extensions.map((ext) => (
            <span
              key={ext}
              className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-slate-100 text-slate-700 text-[10px] rounded font-mono border border-slate-200"
            >
              {ext}
              <button
                type="button"
        ...
        {values.videoMode === "file" && (
          <div className="space-y-3 bg-slate-50/50 p-2.5 rounded-lg border border-slate-100 animate-in fade-in slide-in-from-top-1">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-sm font-medium text-slate-500 mb-1 block">최대 파일 수</label>
                <input
                  type="number"
                  min={1}
                  max={20}
              ...
              <div>
                <label className="text-sm font-medium text-slate-500 mb-1 block">개당 최대 (MB)</label>
                <input
                  type="number"
                  min={1}
            ...
            <div>
              <label className="text-sm font-medium text-slate-500 mb-1 block">전체 최대 용량 (MB)</label>
              <input
                type="number"
                min={1}
                value={values.maxTotalSizeMB ?? 500}
                onChange={(e) =>
                  onChange({ maxTotalSizeMB: Math.max(Number(e.target.value) || 1, values.maxFileSizeMB ?? 500) })
                }
                className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs focus:outline-none focus:border-slate-900 bg-white"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {(["video", "", "custom"] as const).map((mode) => (
                  <label key={mode} className="flex items-center gap-1.5 cursor-pointer group">
                    ...
                    />
                    <span
                      className={`text-[11px] font-medium transition-colors ${(values.fileTypeMode ?? "video") === mode ? "text-slate-900" : "text-slate-500 group-hover:text-slate-700"}`}
                    >
              ...
              {(values.fileTypeMode ?? "video") === "video" && (
                <p className="text-[9.5px] text-slate-400 mt-1 leading-relaxed bg-white border border-slate-200 px-2 py-1 rounded">
                  {FILE_TYPE_LABELS.video}
                </p>
              )}
            </div>
