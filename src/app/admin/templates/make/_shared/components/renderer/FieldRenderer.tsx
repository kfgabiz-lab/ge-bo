...
async function downloadFile(fileId: number, origName: string, errorMessage: string, isEntity?: boolean) {
  try {
    const res = await api.get(isEntity ? `/file-meta/${fileId}/download` : `/page-files/${fileId}`, {
      responseType: "blob",
    });
...
export function FieldRenderer({
  ...
  switch (field.type) {
    ...
    case "text": {
      ...
      if (Array.isArray(fetched)) {
        ...
        if (!isPreview) {
          ...
          formatted = formatFetchedRelValue(
            fetched,
            rowData ?? {},
            field.relationSlugId,
            field.data,
            field.fetchDisplayMode ?? "ONE_LINE"
          );
        }
        return (
          <div
            ...
            <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-md shadow-lg">
              <ul className="max-h-48 overflow-y-auto py-1">
                {opts.map((opt) => {
                  ...
                  return (
                    <li key={opt}>
                      <div className="px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 cursor-default">{t(text)}</div>
                    </li>
                  );
                })}
              </ul>
    ...
    case "date": {
      ...
      const inputType =
        dateSubType === "yearMonth"
          ? "month"
          : dateSubType === "datetime"
            ? "datetime-local"
            : dateSubType === "time" || dateSubType === "timeSec"
              ? "time"
              : "date";
    ...
    case "dateRange": {
      ...
      const subType = field.type === "yearMonthRange" ? "yearMonth" : (field.rangeSubType ?? "date");
    ...
    /* ── button ── */
    case "button": {
      const btnOpts = isPreview ? (field.options?.slice(0, 4) ?? ["전체:all", "오늘:today", "1주:1w"]) : opts;
            ...
            {btnOpts.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-2 text-xs font-medium rounded-md border transition-all ${
                    isActive
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-1.5">
            {existingFileMeta?.map((meta) => (
              <div
                key={meta.id}
                className="flex items-center gap-2 px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-md text-xs flex-shrink-0"
              >
            ...
            {fileList?.map((file, idx) => (
              <div
                key={idx}
                className="flex items-center gap-2 px-2 py-1.5 bg-blue-50 border border-blue-200 rounded-md text-xs flex-shrink-0"
              >
    ...
    case "video": {
            ...
            onDrop={
              canAdd
                ? (e) => {
                    e.preventDefault();
                    const files = Array.from(e.dataTransfer.files);
                    if (files.length > 0) handleVidSelect(files);
                  }
                : undefined
            }
          >
      ...
      return (() => {
        ...
        return (
          ...
          <div
            className="p-1"
            style={{ height: vidHeight, isolation: "isolate" }}
            onDragOver={canAdd ? (e) => e.preventDefault() : undefined}
            onDrop={
              canAdd
                ? (e) => {
              ...
              {displayItems.map((item, i) => {
                if (item.kind === "existing") {
                  ...
                  return (
                    ...
                    <div
                      key={item.meta.id}
                      className="relative border border-slate-200 group flex flex-col"
                    ...
                    >
                      <div className="relative flex-1 min-h-0">
                        {blobUrl ? (
                          ...
                          <div
                            className="w-full flex flex-col items-center justify-center gap-1 bg-slate-50"
                            style={{ height: `${videoAreaH}px` }}
                          >
                      ...
                      <FileInfoBar
                        ...
                        size={item.meta.fileSize}
                        onDownload={() =>
                          downloadFile(item.meta.id, item.meta.origName, t("common.error.file_download"), isEntity)
                        }
                      />
                    </div>
                  );
                ...
                if (item.kind === "new") {
                  ...
                  return (
                    <div
                      key={`new-${item.idx}`}
                      className="relative border border-blue-200 group flex flex-col"
                      style={{ borderRadius: "6px" }}
                    >
                      <div className="relative flex-1 min-h-0">
    ...
    case "media": {
      ...
      const toAccept = (exts: string[]) => exts.map((e) => (e.startsWith(".") ? e : "." + e)).join(",");
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <path
                ...
                d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3 17.25V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v9.75M8.25 12a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z"
              />
            </svg>
            {/* 동영상 아이콘 */}
            <svg
        ...
        return (
          ...
          >
            {existingMedia ? (
              ...
              <div className="flex-1 p-2 flex flex-col gap-1.5 overflow-hidden">
                {mediaFiles.map((file, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="flex items-center gap-2 px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-md text-xs"
                    >
                      {isImg ? (
              ...
              /* preview 빈 상태 */
              <div className="flex-1 flex flex-col items-center justify-center">{mediaPlaceholder}</div>
            )}
          </div>
        );
      }
        ...
        return (
          ...
          >
            ...
            <FileInfoBar
              ...
              onDownload={() =>
                downloadFile(existingMedia.id, existingMedia.origName, t("common.error.file_download"), isEntity)
              }
            />
          </div>
        );
      ...
      return (
        ...
        >
          <div className="flex-1 min-h-0 relative overflow-hidden">
            {selectedIsImg ? (
              /* 이미지 미리보기 */
              <FileImagePreview file={selectedFile} className="w-full h-full object-contain" />
            ) : (
          ...
          <FileInfoBar
            ...
            onDownload={() => downloadLocalFile(selectedFile)}
          />
        </div>
      );
    }
    ...
    case "message-key-select":
      return <MessageKeySelector value={value} onChange={(v) => onChange?.(v)} disabled={isDisabled} />;
    ...
    case "dateRangeStatus": {
      ...
      const beforeLabel = field.beforeTextMsgKey
        ? t(field.beforeTextMsgKey)
        : field.beforeText || t("common.status.before");
      const inRangeLabel = field.inRangeTextMsgKey
        ? t(field.inRangeTextMsgKey)
        : field.inRangeText || t("common.status.inrange");
      ...
      return (
        <div className="flex items-center gap-4 py-2">
          {statusOpts.map((opt) => (
            <label
              key={opt.val}
              className={`flex items-center gap-2 ${isDisabled ? "cursor-default" : "cursor-pointer"}`}
            >
              <input
                type="radio"
                name={`${uid}-field-${field.id}`}
