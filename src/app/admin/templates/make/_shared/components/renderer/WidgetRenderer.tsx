...
async function fetchAndMapFieldValues(
  ...
  if (editId != null && connectedSlug) {
    try {
      ...
      sourceData = typeof res.data.dataJson === "string" ? JSON.parse(res.data.dataJson) : (res.data.dataJson ?? {});
    } catch {
      /* 조회 실패 시 빈 값으로 처리 */
    }
  }
  ...
  const section: Record<string, unknown> =
    ...
    !Array.isArray(sourceData[contentKey])
      ? (sourceData[contentKey] as Record<string, unknown>)
      : sourceData;
...
const BASE_CLS = "h-full w-full rounded border bg-white border-slate-300 shadow-sm overflow-hidden p-2";
...
export function WidgetRenderer({
  ...
  /* inlineEdit 저장 대상 리다이렉트용 slug-relation 목록 — table 위젯에 relation 연동 inlineEdit 컬럼이
   * 있을 때만 조회한다(불필요한 API 호출 방지). widget이 바뀌어도 훅 호출 순서는 항상 동일해야 하므로
   * useSlugRelations 호출 자체는 매 렌더 무조건 실행하고, 내부적으로 enabled 플래그로 fetch만 건너뛴다 */
  const hasInlineEditRelationCol =
    mode === "live" &&
    !!widget &&
    widget.type === "table" &&
    widget.columns.some((c) => c.cellType === "inlineEdit" && c.inlineEditRelationSlugId != null);
  const slugRelations = useSlugRelations(hasInlineEditRelationCol);
  ...
  const [popupSubListRowsMap, setPopupSubListRowsMap] = useState<
    Record<string, import("./SubListRenderer").SubListRow[]>
  >({});
  /* 팝업 내 SubList 파일 맵 — widgetId → rowId → colId → File[] */
  const [popupSubListFileMap, setPopupSubListFileMap] = useState<
  ...
  const popupCfgRef = useRef(popupCfg);
  popupCfgRef.current = popupCfg;
  const popupFormValuesMapRef = useRef(popupFormValuesMap);
  popupFormValuesMapRef.current = popupFormValuesMap;
  ...
  useEffect(() => {
    ...
    return () => {
      _hiddenLogCallbacks.delete(logHidden);
    };
  }, []);
  ...
  const doExcelDownload = useCallback(
    async (
      ...
      try {
        ...
        const exportUrl = isEntity
          ? `${entityApiPath(tableWidget.connectedSlug)}/export`
          : `/page-data/${tableWidget.connectedSlug}/export`;
   ...
   */
  const handleInternalPopupOpen = useCallback(
    async (
      slug: string,
      editId?: number | null,
      _listSlug?: string,
      ...
      try {
        ...
        const tableContents = cfg.widgetItems
          .flatMap((item) => item.contents)
          .filter((c) => c.widget?.type === "table");
        if (tableContents.length > 0) {
          const tableDataAccum: Record<string, PageTableData> = {};
          await Promise.all(
            tableContents.map(async (tc) => {
        ...
        const sublistContents = cfg.widgetItems
          .flatMap((item) => item.contents)
        ...
        if (paramSave && initialValues && Object.keys(initialValues).length > 0) {
          ...
          Object.entries(initialValues).forEach(([key, value]) => {
            ...
            if (dotIdx !== -1) {
              ...
              const hasTable = tableContents.some(
                (c) => (c.widget as { contentKey?: string }).contentKey === contentKey
              );
      ...
      try {
        ...
        setPopupTableDataMap((prev) => ({
          ...
          [widgetId]: {
            ...
            appendLoading: false,
            hasMore: res.data.last === false,
            nextPage: res.data.last === false ? page + 1 : page,
          },
        }));
      } catch {
        setPopupTableDataMap((prev) => ({
  ...
  const handlePopupContentAction = useCallback(
    async (
      ...
      if (action === "delete") {
        if (!popupEditId) {
          toast.info(t("common.table.no_delete_data"));
          return;
        }
      ...
      const subWidgetsForValidation = saveSublistContents.map((c) => c.widget) as Array<{
        type: string;
        widgetId?: string;
        required?: boolean;
        title?: string;
      ...
      for (const c of saveMultiSelectContents) {
        const mw = c.widget as { widgetId?: string; required?: boolean; title?: string };
      ...
      try {
        ...
        for (const fc of saveFormContents) {
          ...
          for (const f of fwFields) {
            ...
            const uploadedIds = newFiles.length
              ? await uploadFiles(newFiles, popupListSlug, f.fieldKey || f.label || "")
              : [];
   ...
   */
  const handlePopupDataSave = useCallback(
    async (
      connectedContentWidgetIds: string[],
      dataSaveSlug: string,
      goBackAfterAction?: boolean,
      ...
      try {
        ...
        if (nonTableWidgets.length > 0) {
          const datasaveAllFormValues = Object.assign({}, ...Object.values(popupFormValuesMap)) as Record<
            string,
            string
          >;
          const { dataJson, pkKeys } = buildDataJson(
      ...
      <div className="px-4 pb-4">
        <PageGridContainer>
          <PageGridRenderer
            ...
            /* 팝업 내 SubList rows */
            subListRowsMap={popupSubListRowsMap}
            onSubListRowsChange={(wid, rows) => setPopupSubListRowsMap((prev) => ({ ...prev, [wid]: rows }))}
  ...
  if (widget.type === "table") {
    ...
    const wrappedHandlers: TableActionHandlers = {
      onEdit: (row) => {
        ...
        if (actionsCol?.editPageRules?.length) {
          ...
          if (matched?.pageSlug) {
            if ((matched.connType ?? "popup") === "popup") {
              ...
              const initialValues = matched.passParam ? parseActionParams(matched.passParam, row) : undefined;
              handleInternalPopupOpen(
                matched.pageSlug,
                row._id as number,
                dataSlug,
                initialValues,
                row._groupId as string | null
              );
            } else {
        ...
        if (slug) {
          const initialValues = actionsCol?.editParams ? parseActionParams(actionsCol.editParams, row) : undefined;
      ...
      onDetail: (row) => {
        ...
        if (slug) {
          const initialValues = actionsCol?.detailParams ? parseActionParams(actionsCol.detailParams, row) : undefined;
          ...
          ? async (row: Record<string, unknown>) => {
              ...
              try {
                ...
                if (copyActionsCol?.copyFixedParams) {
                  ...
                  Object.entries(parseActionParams(copyActionsCol.copyFixedParams, flatOriginal)).forEach(
                    ([key, value]) => {
       ...
       * col.inlineEditRelationSlugId가 설정된 경우, 테이블 자신(connectedSlug)이 아니라
       * 연동 relation의 slave slug로 저장을 리다이렉트한다(예: 카테고리 목록에서 연동된 제품 데이터 필드 수정).
       * slave 레코드의 실제 id는 relation.masterKey 경로로 이 행 자신에서 직접 읽는다
       * (_fetchedRel{id}는 fetch_fields 설정 시 매칭된 필드값만 내려오고 id는 포함하지 않기 때문) */
      onInlineEdit:
        handlers?.onInlineEdit ??
        (mode === "live" && connectedSlug
          ? async (col, row, value) => {
              const fieldKey = col.inlineEditFieldKey;
              if (!fieldKey) return;
              const rowId = (row as { _id?: number })._id;
              if (rowId == null) return;

              /* 저장 대상 slug/id 결정 — relation 미설정 시 기존처럼 테이블 자신에 저장(하위호환) */
              let targetSlug = connectedSlug;
              let targetId = rowId;
              if (col.inlineEditRelationSlugId != null) {
                const relation = slugRelations.find((r) => r.id === col.inlineEditRelationSlugId);
                const slaveId = relation?.masterKey != null ? Number(resolveAccessor(row, relation.masterKey)) : NaN;
                if (!relation || Number.isNaN(slaveId)) {
                  console.error(
                    "[inlineEdit] 연동 relation 또는 slave id를 찾을 수 없습니다.",
                    col.inlineEditRelationSlugId
                  );
                  toast.error(t("common.error.update"));
                  return;
                }
                targetSlug = relation.slaveSlug;
                targetId = slaveId;
              }

              try {
                await api.patch(`/page-data/${targetSlug}/${targetId}/field`, { fieldKey, value });
                /* 성공 후에만 로컬 상태 반영(실패 시 상태 변경 자체가 없어 별도 롤백 불필요) —
                 * 저장 경로(fieldKey)와 화면 표시 accessor(col.accessor)가 다를 수 있어(연동 relation 케이스) 둘 다 갱신 */
                const lastKey = fieldKey.includes(".") ? fieldKey.split(".").pop()! : fieldKey;
                setLocalTableData((prev) =>
                  prev?.map((r) => {
                    if ((r as { _id?: number })._id !== rowId) return r;
                    const updated = applyDotField(r as Record<string, unknown>, fieldKey, value);
                    return { ...updated, [lastKey]: value, [col.accessor]: value };
                  })
                );
                onRefresh?.();
              } catch (e) {
    ...
    return (
      <>
        <SpaceRenderer
          ...
          onClose={onClose}
          onPopupOpen={(slug, params) =>
            handleInternalPopupOpen(slug, null, dataSlug, params ? parseActionParams(params, {}) : undefined)
          }
          onExcelDownload={
            mode === "live"
              ? handleExcelDownload
  ...
  if (widget.type === "category") {
    ...
    const selectedParentId = widget.parentWidgetId ? (categorySelections?.[widget.parentWidgetId] ?? null) : null;
    return (
      <>
        <CategoryRenderer
          mode={mode}
  ...
  if (widget.type === "multiselect") {
    ...
    return (
      <MultiSelectRenderer
        ...
        onExtraFieldChange={(itemId, fieldKey, value) =>
          onMultiSelectExtraFieldChange?.(msWid, itemId, fieldKey, value)
        }
      />
    );
  }
