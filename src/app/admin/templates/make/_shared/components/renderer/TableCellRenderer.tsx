  ...
  switch (col.cellType) {
    ...
    case "boolean": {
      ...
      const boolText = boolVal
        ? col.trueTextMsgKey
          ? t(col.trueTextMsgKey)
          : col.trueText || t("common.label.public")
        : col.falseTextMsgKey
    ...
    case "dateRangeStatus": {
      ...
      const inRangeLabel = col.inRangeTextMsgKey
        ? t(col.inRangeTextMsgKey)
        : col.inRangeText || t("common.status.inrange");
    ...
    case "inlineEdit": {
      ...
      /* 현재 값은 위에서 이미 accessor/data 기준으로 구해둔 value를 그대로 사용한다.
       * (저장 경로 inlineEditFieldKey는 "저장"에만 쓰고, 화면 표시는 항상 accessor 기준 — 서로 다를 수 있음.
       *  예: accessor='_fetchedRel15'(연동 slug에서 가져온 표시값), inlineEditFieldKey='productDataForm.trainingYn'(연동 slug 저장 경로)) */
      ...
      const codeDetails = col.codeGroupCode
        ...
        : [];
      const resolvedOpts = codeDetails.length > 0 ? codeDetails.map((d) => ({ text: d.name, value: d.code })) : opts;

      /* ── 토글 ── */
      if (editType === "toggle") {
        /* true/false 값 매핑 — 두 값이 모두 설정된 경우만 값 비교로 판정(예: 001/002 같은 공통코드 값),
         * 미설정 시 기존처럼 Boolean(value)로 판정(하위호환) */
        const hasValueMap = !!col.inlineEditTrueValue && !!col.inlineEditFalseValue;
        const boolVal = isPreview
          ? rowIndex % 2 === 0
          : hasValueMap
            ? String(value) === col.inlineEditTrueValue
            : Boolean(value);
        /* 클릭 시 다음 값 — 값 매핑이 있으면 매핑된 문자열, 없으면 반전된 boolean */
        const nextValue = hasValueMap ? (boolVal ? col.inlineEditFalseValue : col.inlineEditTrueValue) : !boolVal;
        return (
          <button
            type="button"
            onClick={
      ...
      /* ── 라디오 ── */
      if (editType === "radio") {
        const currentVal = isPreview
          ? (resolvedOpts[rowIndex % Math.max(resolvedOpts.length, 1)]?.value ?? "")
          : String(value ?? "");
        ...
        return (
          <div className="flex items-center gap-2 flex-wrap">
            {resolvedOpts.map((opt) => (
              <label
                key={opt.value}
                className={`flex items-center gap-1 ${isPreview ? "cursor-default" : "cursor-pointer"}`}
              >
                <input
                  type="radio"
                  checked={currentVal === opt.value}
                  onChange={
                    !isPreview && col.inlineEditFieldKey
                      ? () => handlers?.onInlineEdit?.(col, row, opt.value)
                      : undefined
                  }
                  disabled={isPreview}
        ...
        if (!resolvedOpts.length) {
          ...
          return (
            <input
              ...
              checked={boolVal}
              onChange={
                !isPreview && col.inlineEditFieldKey ? () => handlers?.onInlineEdit?.(col, row, !boolVal) : undefined
              }
              disabled={isPreview}
                    ...
                    onChange={
                      !isPreview && col.inlineEditFieldKey
                        ? () => {
                            ...
                            handlers?.onInlineEdit?.(col, row, next);
                          }
                        : undefined
                    }
                    disabled={isPreview}
    ...
    default: {
      ...
      if (Array.isArray(value) && col.relationSlugId) {
        ...
        const formatted = formatFetchedRelValue(
          ...
          col.relationSlugId,
          col.data,
          col.fetchDisplayMode ?? "ONE_LINE"
        );
        if (!formatted) return <span className="text-sm text-slate-400">-</span>;
        if (col.fetchDisplayMode === "MULTI_LINE") {
          return (
            <span className="text-sm text-slate-700 whitespace-pre-wrap block" title={formatted}>
              {formatted}
            </span>
          );
        }
