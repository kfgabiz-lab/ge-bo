...
export function TableRenderer({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const inContainerView = sentinelRect.top < containerRect.bottom && sentinelRect.bottom > containerRect.top;
            ...
            : totalElements > 0
              ? isScroll
                ? t("common.pagination.showing", {
                    start: "1",
                    end: String(Math.min((currentPage + 1) * (pageSize || DEFAULT_PAGE_SIZE), totalElements)),
                  })
                : t("common.pagination.showing", {
                    start: String(currentPage * (pageSize || DEFAULT_PAGE_SIZE) + 1),
                    end: String(Math.min((currentPage + 1) * (pageSize || DEFAULT_PAGE_SIZE), totalElements)),
                  })
              : ""}
        </p>
            ...
            {hasMore ? (
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-[10px] text-slate-400 font-medium tracking-tight">
                  {t("common.pagination.loading_more")}
                </span>
              </div>
            ) : data.length > 0 ? (
              <div className="py-2">
              ...
              [1, 2, 3].map((p) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 text-xs rounded border transition-all ${
                    p === 1 ? "bg-slate-900 text-white border-slate-900" : "border-slate-200 text-slate-600"
