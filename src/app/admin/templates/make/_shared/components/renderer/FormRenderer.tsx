...
export function FormRenderer({
  ...
  const shouldHide = (f: FormFieldItem): boolean => !isPreview && !!f.hideCondition && evalCondition(f.hideCondition);
  ...
  const handleFieldChange = useCallback(
    ...
    [fields, keyToId, allFieldKeyToId, onChangeValues, onChangeAllFormValues, contentKey, values]
  );
                  ...
                  !isPreview && f.fieldKey
                    ? (derived) => {
                        Object.entries(derived).forEach(([derivedKey, v]) => {
                          onDerivedValueChange?.(`${f.fieldKey}.${derivedKey}`, v);
                        });
                      }
                    : undefined
                }
                valueFrom={
                  f.type === "dateRange" || f.type === "yearMonthRange" ? (values?.[f.id + "_from"] ?? "") : undefined
                }
                valueTo={
                  f.type === "dateRange" || f.type === "yearMonthRange" ? (values?.[f.id + "_to"] ?? "") : undefined
                }
                onFromChange={
                  f.type === "dateRange" || f.type === "yearMonthRange"
                    ? isPreview
