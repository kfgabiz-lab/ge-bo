...
export type PageRelationDirection = "in" | "out";

export interface PageRelationLink {
  relationId: number;
  direction: PageRelationDirection;
}
...
export interface OutputModeValue {
  ...
  setPageRelations: (v: PageRelationLink[]) => void;
  /**
...
export function useOutputMode(): OutputModeValue {
  ...
  const [pageRelations, setPageRelations] = useState<PageRelationLink[]>([]);
  ...
  const restore = (cfg: Record<string, unknown>) => {
    ...
    setConnectedType(
      (cfg.connectedType as ConnectedType) ??
        ((cfg.slugEntityId as number) ? "entity" : (cfg.mainConnectedSlug as string) ? "slug" : "none")
    );
    setPageRelations(Array.isArray(cfg.pageRelations) ? (cfg.pageRelations as PageRelationLink[]) : []);
  };
