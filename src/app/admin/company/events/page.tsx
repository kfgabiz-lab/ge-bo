...
const SEARCH_FIELDS_Search1: SearchFieldConfig[] = [
  {
    colSpan: 1,
    id: "events.is_visible",
    type: "select",
    fieldKey: "events.is_visible",
    label: "",
  },
  {
    colSpan: 4,
...
const DETAIL_PAGE_PATH = "/admin/company/events/detail";
...
export default function GeneratedPage() {
  ...
  const { t } = useI18n();
  useEffect(() => {
    setPageTitle(t("common.label.event"));
  }, [setPageTitle, t]);
  const { groups, fetchGroups } = useCodeStore();
  useEffect(() => {
    fetchGroups();
  }, [fetchGroups]);
  const initialParamsSearch1: Record<string, string> = { "events.is_visible": "", "events.title": "" };
  ...
  const dataSlugTable1 = "events-data";
                ...
                >
                  <div className="col-span-1">
                    <div className="relative">
                      <select
                        value={String(paramsSearch1["events.is_visible"] ?? "")}
                        onChange={(e) =>
                          setParamsSearch1((prev) => ({ ...prev, ["events.is_visible"]: e.target.value }))
                        }
                  ...
                  <div className="col-span-4">
                    <input
                      type="text"
                      value={String(paramsSearch1["events.title"] ?? "")}
                      onChange={(e) => setParamsSearch1((prev) => ({ ...prev, ["events.title"]: e.target.value }))}
                      placeholder={t("events.placeholder.search")}
                  ...
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10">
                      <tr className="border-b border-slate-200 bg-slate-50/80">
                        ...
                        >
                          <button
                            onClick={() => handleSortTable1("period_from")}
                            className="flex items-center justify-center gap-1 w-full transition-colors hover:text-slate-900"
                          >
                            {t("common.label.startDate")}
                            {(sortKeyTable1 === "period_from" ? sortDirTable1 : false) === "asc" ? (
                              <ChevronUp className="w-3.5 h-3.5 text-blue-500" />
                            ) : (sortKeyTable1 === "period_from" ? sortDirTable1 : false) === "desc" ? (
                              <ChevronDown className="w-3.5 h-3.5 text-blue-500" />
                            ) : (
                              <ChevronsUpDown className="w-3.5 h-3.5 text-gray-300" />
                            )}
                        ...
                        >
                          <button
                            onClick={() => handleSortTable1("period_to")}
                            className="flex items-center justify-center gap-1 w-full transition-colors hover:text-slate-900"
                          >
                            {t("common.label.endDate")}
                            {(sortKeyTable1 === "period_to" ? sortDirTable1 : false) === "asc" ? (
                              <ChevronUp className="w-3.5 h-3.5 text-blue-500" />
                            ) : (sortKeyTable1 === "period_to" ? sortDirTable1 : false) === "desc" ? (
                              <ChevronDown className="w-3.5 h-3.5 text-blue-500" />
                            ) : (
                              <ChevronsUpDown className="w-3.5 h-3.5 text-gray-300" />
                            )}
                      ...
                      {loadingTable1 ? (
                        ...
                        rowsTable1.map((row, idx) => (
                          ...
                          >
                            ...
                            >
                              {(() => {
                                const value = row["period_from"];
                                const dateVal = formatCellDate(String(value ?? ""), "YYYY-MM-DD");
                                return (
