...
import { Image as ImageIcon, Plus, X, Calendar } from "lucide-react";
  ...
  fields: [
    {
      ...
      colSpan: 8,
      rowSpan: 1,
      labelMsgKey: "events.info.title",
      minLength: 3,
      maxLength: 150,
      required: true,
      placeholderMsgKey: "events.placeholder.title",
      showCharCount: true,
      dataGenerations: [
        {
    ...
    },
    {
      ...
      descriptionMsgKey: "events.description.image",
      maxFileSizeMB: 3,
      required: false,
      imageMaxWidthPx: 708,
      imageMaxHeightPx: 445,
    },
    {
      id: "fb_iuer3vw65",
      type: "input",
      ...
      colSpan: 8,
      rowSpan: 1,
      labelMsgKey: "events.label.location",
      required: true,
      minLength: 1,
      maxLength: 100,
      placeholderMsgKey: "events.placeholder.title",
    ...
    },
    {
      ...
      rowSpan: 1,
      rangeSubType: "date",
      labelMsgKey: "events.label.period",
      required: true,
...
export default function GeneratedPage() {
  ...
  const { t } = useI18n();
  useEffect(() => {
    setPageTitle(t("events.label.title"));
  }, [setPageTitle, t]);
  ...
  useEffect(() => {
    if (storedId === null) {
      ...
      setFormValuesForm1(defaults["w_8mchlsfnn"] ?? {});
      setFormValuesForm2(defaults["w_d2w7qx0ca"] ?? {});
      return;
    }
    let cancelled = false;
    api
      .get(`/page-data/events-data/${storedId}`)
      .then(async (res) => {
        if (cancelled) return;
        ...
        setFormValuesForm1((prev) => ({ ...prev, ...(valuesByWidgetId["w_8mchlsfnn"] ?? {}) }));
        setFormValuesForm2((prev) => ({ ...prev, ...(valuesByWidgetId["w_d2w7qx0ca"] ?? {}) }));
        const fileIds = collectFileIdsDeep(dataJson);
        if (fileIds.length === 0) return;
        const metaList = await fetchFileMetaByIds(fileIds, false);
        const sectionForm1 = findSection(dataJson, "events");
    ...
    try {
      ...
      try {
        ...
        if (savedFileIds.length > 0) {
          ...
          const savedSectionForm1 = dataJson["events"] as Record<string, unknown>;
  ...
  return (
    <div className="space-y-3">
      <PageGridContainer>
        <GridCell colSpan={12} rowSpan={22} autoHeight>
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "repeat(12, 1fr)",
              gridTemplateRows: `auto auto auto auto auto auto auto auto auto auto auto auto auto auto auto auto auto auto auto auto auto auto`,
          ...
          >
            <div style={{ gridColumn: "span 12", gridRow: "span 15" }}>
              {/* TODO(파일빌드): 처리되지 않은 설정 값이 있습니다 (field:rangeSubType,defaultStartToday). 필요 시 직접 구현해주세요. */}
              <div
              ...
              >
                  ...
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    <div className="relative">
                      <input
                        ...
                        value={formValuesForm1["fb_926blopy2"] ?? ""}
                        onChange={(e) => handleFieldChangeForm1("fb_926blopy2", e.target.value)}
                        onBlur={() => handleFieldBlurForm1("fb_926blopy2")}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 pointer-events-none">
                        {(formValuesForm1["fb_926blopy2"] ?? "").length}/{150}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="flex flex-col px-3 min-w-0" style={{ gridColumn: "span 8", gridRow: "span 3" }}>
                  ...
                  <p className="text-sm text-slate-400 mb-0.5 flex-shrink-0 leading-tight whitespace-nowrap overflow-x-auto min-h-[18px]">
                    {t("events.description.image")}
                  </p>
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    {(() => {
                      const maxCount = 1;
                      const existingList = existingFileMetaForm1["fb_i96wuphru"] ?? [];
                      const newList = fileValuesForm1["fb_i96wuphru"] ?? [];
                      ...
                      const handleImgSelect = async (selected: File[]) => {
                        ...
                        for (const file of valid) {
                          if (file.size > 3 * unitToBytes("MB")) {
                            toast.warning(
                              t("common.field.file_size_limit", { type: t("common.label.image"), mb: "3MB" })
                            );
                            continue;
                          }
                          const naturalSize = await getImageNaturalSize(file);
                          const violation = checkImagePixelLimit(naturalSize, 708, 445);
                          if (violation === "width") {
                            toast.warning(t("common.field.image_width_limit", { label: file.name, px: "708" }));
                            continue;
                          }
                          if (violation === "height") {
                            toast.warning(t("common.field.image_height_limit", { label: file.name, px: "445" }));
                            continue;
                          }
                          passed.push(file);
                        }
                        if (passed.length > 0)
                          handleFileChangeForm1("fb_i96wuphru", [...newList, ...passed].slice(0, maxCount));
                      };
                                    ...
                                    return (
                                      ...
                                      >
                                        <div className="relative flex-1 min-h-0">
                                          ...
                                          <button
                                            type="button"
                                            onClick={() => handleRemoveExistingForm1("fb_i96wuphru", item.meta.id)}
                  ...
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    <div className="relative">
                      <input
                        ...
                        maxLength={100}
                        className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed disabled:border-slate-200 pr-20"
                        value={formValuesForm1["fb_iuer3vw65"] ?? ""}
                        onChange={(e) => handleFieldChangeForm1("fb_iuer3vw65", e.target.value)}
                        onBlur={() => handleFieldBlurForm1("fb_iuer3vw65")}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 pointer-events-none">
                        {(formValuesForm1["fb_iuer3vw65"] ?? "").length}/{100}
                      </span>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          disabled={false}
                          className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed disabled:border-slate-200 pl-9"
                          value={formValuesForm1["fb_encm1xsl6_from"] ?? ""}
                          onChange={(e) => handleFieldChangeForm1("fb_encm1xsl6_from", e.target.value)}
                      ...
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          disabled={false}
                          className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed disabled:border-slate-200 pl-9"
                          value={formValuesForm1["fb_encm1xsl6_to"] ?? ""}
                          onChange={(e) => handleFieldChangeForm1("fb_encm1xsl6_to", e.target.value)}
                          onClick={(e) => e.currentTarget.showPicker?.()}
                        />
                      </div>
                    </div>
                ...
                <div className="flex flex-col px-3 min-w-0" style={{ gridColumn: "span 8", gridRow: "span 6" }}>
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    <TiptapEditor
                      initialValue={formValuesForm1["fb_lfepbt1s7"] ?? ""}
                      onChange={(v: string) => handleFieldChangeForm1("fb_lfepbt1s7", v)}
                      height="528px"
                    />
                  </div>
                </div>
                <div className="flex flex-col px-3 min-w-0" style={{ gridColumn: "span 8", gridRow: "span 1" }}>
                  ...
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    <input
                      ...
                      className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed disabled:border-slate-200"
                      value={formValuesForm1["fb_wqv0iit76"] ?? ""}
                      min={formatNowBySubType("datetime") || formatNowBySubType("datetime")}
                      onChange={(e) => handleFieldChangeForm1("fb_wqv0iit76", e.target.value)}
                      onClick={(e) => e.currentTarget.showPicker?.()}
                    />
                  </div>
                </div>
                <div className="flex flex-col px-3 min-w-0" style={{ gridColumn: "span 8", gridRow: "span 1" }}>
                  ...
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    <div className="flex items-center gap-4">
                      {resolveFieldOptions(
                        FORM_FIELD_BY_ID_Form1["fb_wc9un3qwz"] as unknown as SearchFieldConfig,
                        groups
                      ).map((opt) => {
                        ...
                        return (
                          <label key={opt} className="flex items-center gap-2 cursor-pointer">
                            <input
                              type="radio"
                              name={`${uid}-field-fb_wc9un3qwz`}
                              disabled={false}
                              value={parsed.value}
                              checked={(formValuesForm1["fb_wc9un3qwz"] ?? "") === parsed.value}
                              onChange={() => handleFieldChangeForm1("fb_wc9un3qwz", parsed.value)}
                  ...
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    <div className="relative">
                      <input
                        ...
                        maxLength={150}
                        className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed disabled:border-slate-200 pr-20"
                        value={formValuesForm2["fb_ogcligloe"] ?? ""}
                        onChange={(e) => handleFieldChangeForm2("fb_ogcligloe", e.target.value)}
                        onBlur={() => handleFieldBlurForm2("fb_ogcligloe")}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 pointer-events-none">
                        {(formValuesForm2["fb_ogcligloe"] ?? "").length}/{150}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    <div className="relative">
                      <input
                        ...
                        maxLength={150}
                        className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all bg-white disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed disabled:border-slate-200 pr-20"
                        value={formValuesForm2["fb_47m39xuy5"] ?? ""}
                        onChange={(e) => handleFieldChangeForm2("fb_47m39xuy5", e.target.value)}
                        onBlur={() => handleFieldBlurForm2("fb_47m39xuy5")}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 pointer-events-none">
                        {(formValuesForm2["fb_47m39xuy5"] ?? "").length}/{150}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex-1 min-h-0 flex flex-col justify-center-safe">
                    <div className="flex flex-col h-full">
                      <textarea
                        ...
                        value={formValuesForm2["fb_roibxaxq4"] ?? ""}
                        maxLength={180}
                        placeholder={t("common.placeholder.seo.metaDescription")}
                        onChange={(e) => handleFieldChangeForm2("fb_roibxaxq4", e.target.value)}
                      />
                      <div className="text-right text-[10px] text-slate-400 mt-0.5">
                        {(formValuesForm2["fb_roibxaxq4"] ?? "").length}/{180}
                      </div>
                    </div>
                  </div>
                </div>
