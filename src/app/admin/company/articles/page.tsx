...
const SEARCH_FIELDS_Search1: SearchFieldConfig[] = [
  {
    colSpan: 1,
    id: "articles.is_visible",
    type: "select",
    fieldKey: "articles.is_visible",
    label: "",
  },
  {
    colSpan: 1,
  ...
  },
  {
    ...
    id: "articles.title",
    type: "input",
    fieldKey: "articles.title",
    label: "",
  },
];
const searchKeyToIdSearch1 = buildKeyToId(SEARCH_FIELDS_Search1);
const DETAIL_PAGE_PATH = "/admin/company/articles/detail";
...
export default function GeneratedPage() {
  ...
  const { t } = useI18n();
  useEffect(() => {
    setPageTitle(t("common.label.articles"));
  }, [setPageTitle, t]);
  const { groups, fetchGroups } = useCodeStore();
  useEffect(() => {
    fetchGroups();
  }, [fetchGroups]);
  const initialParamsSearch1: Record<string, string> = { "articles.is_visible": "", status: "", "articles.title": "" };
  ...
  const dataSlugTable1 = "articles-data";
    ...
    (async () => {
      try {
        ...
        const normalizedBase = normalizeExternalUrl(
          "https://nahpdev-web.ls-electric.com/company/articles/detail/"
        ).replace(/\/$/, "");
                ...
                >
                  <div className="col-span-1">
                    <div className="relative">
                      <select
                        value={String(paramsSearch1["articles.is_visible"] ?? "")}
                        onChange={(e) =>
                          setParamsSearch1((prev) => ({ ...prev, ["articles.is_visible"]: e.target.value }))
                        }
                  ...
                  <div className="col-span-3">
                    <input
                      type="text"
                      value={String(paramsSearch1["articles.title"] ?? "")}
                      onChange={(e) => setParamsSearch1((prev) => ({ ...prev, ["articles.title"]: e.target.value }))}
                          ...
                          >
                            ...
                            >
                              {(() => {
                                ...
                                const strVal = value == null || typeof value === "object" ? "" : String(value);
                                const displayVal = resolveCodeLabel(strVal, "PUBLISHSTATUS", "text", groups, t);
                                return (
                                  <span className="text-sm text-slate-700 truncate block" title={displayVal}>
                                    {displayVal}
                                  </span>
