...
const SEARCH_FIELDS_Search1: SearchFieldConfig[] = [
  {
    colSpan: 1,
    id: "careers.is_visible",
    type: "select",
    fieldKey: "careers.is_visible",
    label: "공ㄱ",
  },
  {
    colSpan: 4,
    id: "careers.title",
    type: "input",
    fieldKey: "careers.title",
    label: "",
  },
];
const searchKeyToIdSearch1 = buildKeyToId(SEARCH_FIELDS_Search1);
const DETAIL_PAGE_PATH = "/admin/company/careers/detail";
...
export default function GeneratedPage() {
  ...
  const { t } = useI18n();
  useEffect(() => {
    setPageTitle(t("common.label.careers"));
  }, [setPageTitle, t]);
  const { groups, fetchGroups } = useCodeStore();
  useEffect(() => {
    fetchGroups();
  }, [fetchGroups]);
  const initialParamsSearch1: Record<string, string> = { "careers.is_visible": "", "careers.title": "" };
  ...
  const dataSlugTable1 = "careers-data";
      ...
      <PageGridContainer>
        <GridCell colSpan={12} rowSpan={12} autoHeight>
          ...
          >
                ...
                >
                  <div className="col-span-1">
                    <div className="relative">
                      <select
                        value={String(paramsSearch1["careers.is_visible"] ?? "")}
                        onChange={(e) =>
                          setParamsSearch1((prev) => ({ ...prev, ["careers.is_visible"]: e.target.value }))
                        }
                  ...
                  <div className="col-span-4">
                    <input
                      type="text"
                      value={String(paramsSearch1["careers.title"] ?? "")}
                      onChange={(e) => setParamsSearch1((prev) => ({ ...prev, ["careers.title"]: e.target.value }))}
            ...
            <div style={{ gridColumn: "11 / span 2", gridRow: "span 1" }}>
              <div
                  ...
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 z-10">
                      <tr className="border-b border-slate-200 bg-slate-50/80">
                        ...
                        >
                          <button
                            onClick={() => handleSortTable1("careers.sort_order")}
                            className="flex items-center justify-center gap-1 w-full transition-colors hover:text-slate-900"
                          >
                            {t("common.label.order")}
                            {(sortKeyTable1 === "careers.sort_order" ? sortDirTable1 : false) === "asc" ? (
                              <ChevronUp className="w-3.5 h-3.5 text-blue-500" />
                            ) : (sortKeyTable1 === "careers.sort_order" ? sortDirTable1 : false) === "desc" ? (
                        ...
                        >
                          <button
                            onClick={() => handleSortTable1("careers.title")}
                            className="flex items-center justify-center gap-1 w-full transition-colors hover:text-slate-900"
                          >
                            {t("common.label.title")}
                            {(sortKeyTable1 === "careers.title" ? sortDirTable1 : false) === "asc" ? (
                              <ChevronUp className="w-3.5 h-3.5 text-blue-500" />
                            ) : (sortKeyTable1 === "careers.title" ? sortDirTable1 : false) === "desc" ? (
                              <ChevronDown className="w-3.5 h-3.5 text-blue-500" />
                            ) : (
                              <ChevronsUpDown className="w-3.5 h-3.5 text-gray-300" />
                            )}
                        ...
                        >
                          <button
                            onClick={() => handleSortTable1("careers.is_visible")}
                            className="flex items-center justify-center gap-1 w-full transition-colors hover:text-slate-900"
                          >
                            {t("common.label.isVisible")}
                            {(sortKeyTable1 === "careers.is_visible" ? sortDirTable1 : false) === "asc" ? (
                              <ChevronUp className="w-3.5 h-3.5 text-blue-500" />
                            ) : (sortKeyTable1 === "careers.is_visible" ? sortDirTable1 : false) === "desc" ? (
                              <ChevronDown className="w-3.5 h-3.5 text-blue-500" />
                            ) : (
                              <ChevronsUpDown className="w-3.5 h-3.5 text-gray-300" />
                            )}
                      ...
                      {loadingTable1 ? (
                        ...
                        rowsTable1.map((row, idx) => (
                          ...
                          >
                            ...
                            >
                              {(() => {
                                const value = row["careers.sort_order"];
                            ...
                            >
                              {(() => {
                                const value = row["careers.is_visible"];
                                const strVal = value == null || typeof value === "object" ? "" : String(value);
