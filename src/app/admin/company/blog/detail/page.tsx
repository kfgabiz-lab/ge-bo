...
const FORM_WIDGET_Form1: FormWidget = {
  ...
  fields: [
    ...
    },
    {
      ...
      descriptionMsgKey: "blog.description.image",
      maxFileSizeMB: 5,
      required: false,
      imageMaxWidthPx: 700,
    ...
    },
    {
      ...
      options: ["산업 자동화 및 제어:001", "전력 배전 및 인프라:002", "보호 및 안전:003", "모터 제어 및 전력 전자:004"],
      codeGroupCode: "BLOGCATEGORY",
      descriptionMsgKey: "blog.description.category",
      required: true,
    },
    {
      id: "fb_eb6kcymp7",
      ...
      colSpan: 8,
      rowSpan: 1,
      labelMsgKey: "common.label.hashtag",
      descriptionMsgKey: "common.description.hashtag",
      required: false,
      placeholderMsgKey: "common.placeholder.hashtag",
      minLength: 1,
    },
    {
      id: "fb_csti2aujo",
    ...
    },
    {
      ...
      descriptionMsgKey: "blog.description.visible",
...
export default function GeneratedPage() {
  ...
  const { t } = useI18n();
  useEffect(() => {
    setPageTitle(t("blog.label.title"));
  }, [setPageTitle, t]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get(`/page-data/blog-data/${storedId}`)
      .then(async (res) => {
        if (cancelled) return;
        ...
        setFormValuesForm2((prev) => ({ ...prev, ...(valuesByWidgetId["w_msa446cfj"] ?? {}) }));
        const selectionMultiSelect1 = extractMultiSelectSelection(dataJson, "product_list", "blog-data");
        ...
        const sectionForm1 = findSection(dataJson, "blog");
  ...
  const handleContentActionSpace1_1 = async () => {
    ...
    const missingMultiSelectTitle = findMissingRequiredMultiSelect(
      ...
      { w_8k8qsiwzz: multiSelectIdsMultiSelect1 },
      allFieldKeyToId,
      allFormValues,
      t
    );
    ...
    try {
      ...
      try {
        ...
        if (savedFileIds.length > 0) {
          ...
          const savedSectionForm1 = dataJson["blog"] as Record<string, unknown>;
